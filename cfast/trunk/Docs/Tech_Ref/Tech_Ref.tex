...
In scenarios where a compartment with a hot gas layer opens to another compartment overhead via a horizontal (ceiling) vent and the density difference, $\Delta \rho$, between the lower layer of the upper compartment and the upper layer of the lower compartment is greater than zero, the volumetric flow rate, $\dot{V}$, through the vent is given by Cooper~\cite{Cooper:1989}:
\be
   \dot{V} = 0.10 \, \left( \frac{g \, \Delta \rho \, A_{\rm v}^{5/2}}{\rho_{\rm avg}} \right)^{1/2} \left( 1 - \frac{|\Delta P|}{|\Delta P_{\rm \tiny flood}|} \right) \quad ; \quad 
   |\Delta P_{\rm \tiny flood}| = \frac{C_{\rm s}^2 \, g \, \Delta \rho \, D^5}{2 \, A_{\rm v}^2 }
\ee
Here $A_{\rm v}$ is the area of the vent, $\rho_{\rm avg}$ is the average of the densities above and below the vent, $|\Delta P|$ is the difference in compartment pressures and is less than the value of $|\Delta P_{\rm \tiny flood}|$, $D = 2 \sqrt{A_{\rm v} / \pi}$, and the shape factor $C_{\rm s}=0.754$ for round or 0.942 for square openings.

Consider the case where there is a vent in the ceiling of the lower compartment with a hot gas layer. If the pressure difference between the lower and upper compartments is relatively small, then $\dm_{\rm v}$ will be positive. If the temperature of the hot gas layer of the lower compartment is greater than the temperature of the lower layer of the upper compartment, then the mass and enthalpy flows are deposited into the upper layer of the upper compartment. Otherwise, the flows are deposited into the lower layer of the upper compartment. The enthalpy flow is assumed to be a weighted average of heat extracted from the lower and upper layers of the lower compartment:
\be
  \doh_{\rm v} = \alpha \; c_p \, \dm_{\rm v} \, \Tu + (1-\alpha) \; c_p \, \dm_{\rm v} \, \Tl  \quad ; \quad \alpha = \min \left( \frac{0.5 \, \rho_{\rm u} V_{\rm u}}{\dm_{\rm v} \, \delta t} \; , \; 1 \right)
\ee
The weighting factor, $\alpha$, assures that the amount of mass extracted from the upper layer over a specified time interval, $\delta t=1$~s, does not exceed half of its current mass. The factor of one half and the 1~s time interval are somewhat arbitrary, meant to prevent the entire upper layer from being drawn into the upper compartment.
