...
The configuration factor, $F_{1-2}$, is the fraction of radiant energy emitted by surface 1 that is intercepted by  surface 2, and is given by
\be
   F_{1-2} = \frac{1}{A_1} \int_{A_1} \int_{A_2} \frac{\cos \theta_1 \, \cos \theta_2}{\pi L^2} \, dA_2 \, dA_1 \label{eq:config_factor}
\ee
where $L$ is the distance along the line of integration,  $\theta_1$ and $\theta_2$ are the angles for surface 1 and 2 between the respective normal vectors and the line of integration, and $A_1$ and $A_2$ are the areas of the two surfaces.

\begin{figure}[!ht]
\label{configfactorsetup}
\centering
\includegraphics[width=3.0in]{FIGURES/configfactorsetup}
\caption{Setup for configuration factor computation.}
\end{figure}

As illustrated in Figure \ref{configfactorsetup}, a compartment has four wall surfaces, a ceiling, upper wall, lower wall and floor.  There are then sixteen configuration factor required for a radiation exchange computation between these four surfaces.  The formula for configuration factors between parallel plates (see Appendix~C of Ref.~\cite{SiegelandHowell:1981}) given by
...
\end{eqnarray}
where $a$ and $b$ are the rectangle dimensions, $c$ is the separation distance, $X=a/c$ and $Y=b/c$ is used to compute configuration factors between the floor and ceiling denoted $F_{1-4}$, between the ceiling and layer interface denoted $F_{1-d}$ and between the floor and layer interface denoted $F_{4-d}$. These three factors are then used to compute the 16 factors as follows.

The 16 configuration factors can be determined in terms of $F_{1-4}$, $F_{1-d}$ and $F_{4-d}$. $F_{1-4}$
does not change during a simulation since its value depends only on the height of the room
and the area of the
floor. Therefore, $F_{1-4}$ only needs to be computed once. Configuration
factors, $F_{1-d}$ and $F_{4-d}$ depend on the layer interface height so need to be calculated each
time the radiation exchange is to be calculated. Configuration factors $F_{1-4}$, $F_{1-d}$ and $F_{4-d}$. $F_{1-4}$
are determined using equation (13). Since $A_1 = A_4$ it follows that $F_{4-1} = F_{1-4}$. The other
14 configuration factors can be calculated using simple algebraic formulas.
Since the
floor and the ceiling is assumed to be a
at rectangular surface it follows that $F_{1-1}=F_{4=4}=0$. 
Using the fact that configuration factors in an enclosure sum to 1 and that due to symmetery
...
Equations (x) and (xx) can be solved for $F_{1-2}$ and $F_{2-2}$ to obtain
\begin{eqnarray*}
F_{1-2}&=&1-F_{1-d}\\
...
F_{3-3}&=&1-2F_{3-4}
\end{eqnarray*}
Using the above configuration factors and Eq. (xxx) it follows that
\begin{eqnarray*}
...
\centering
\includegraphics[width=3.0in]{FIGURES/solidanglesetup}
\caption{Setup for configuration factor computation.}
\end{figure}
...
To compute a solid angle, consider an origin $v_0$ and a triangle defined by vertices $v_1$, $v_2$ and $v_3$ as illustrated in Figure \ref{solidanglesetup}.  The solid angle of this triangle may be computed using
\begin{eqnarray*}
...
\be
   ...
   \displaystyle\sum_{f=1}^{n_{\rm fires}} \cos \, \theta_f \; \frac{\chi_{\rm r} \, \dQ_f}{4 \pi R_f^2}  + \displaystyle\sum_{j=1}^{10} \tau_j \, F_j \, \epsilon_j \, q''_{{\rm out},j}  +  \displaystyle\sum_{j=1}^2 \left(\tau_j \, F_j \, \epsilon_j \sigma \, T_j^4 \right)  -\sigma T_{\rm t}^4  \right)  \label{target_rad}
\ee
where $\epsilon_{\rm t}$ is the emissivity of the target, $\epsilon_{\rm j}$ is the emissivity of the wall surface, $\theta_f$ is the angle formed by the line of sight segment (between the fire $f$ and the target) of length $R_f$ and the normal direction to the target surface, $F_j$ and $F_{\rm u}$ are view factors for the walls and upper layer, $\tau_j$ and $\tau_{\rm u}$ are the values of transmittance for the path between the target and the walls and upper layer and $T_{\rm t}$ is the temperature of the target.  The term $q''_{{\rm out},j}=\sigma T_j^4-\frac{1-\epsilon_j}{\epsilon_j}\dq_{{\rm r},j}''$ is the radiative flux leaving the $j$'th wall surface (See Eq.~(17-15) in Siegel and Howell~\cite{SiegelandHowell:1981})\ where $\dq_{{\rm r},j}''$ is the net radiative flux striking the wall surface found using Eq.~(\ref{RTE}).
