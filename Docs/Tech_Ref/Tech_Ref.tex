...
The set of governing equations start with the conservation of mass which may be expressed as the net rate of mass added to a control volume, $\dmi$, is equal to the rate at which mass accumulates in that control volume, $\dbydt{\mi}$ or equivalently
\be
   \dbydt{\mi} = \dmi  \label{mass_con}
\ee
where $\dmi$ represents the sum of all mass flow terms, such as plume mass entrainment and ventilation, entering and leaving layer $i$. Similarly, the conservation of energy may be stated as the net rate that heat $\dqi$ is added to a control volume minus the work $P\dbydt{\Vi}$ performed by the control volume (since the control volume may change size), is equal to the rate at which heat or internal energy $\ddt\left(\cv \mi \Ti\right)$ accumulates within the control volume or equivalently
\be
   \ddt\left(\cv \mi \Ti\right)   =  \dqi - P \dbydt{\Vi}\label{eq:first_law}
\ee

Stated in this way, conservation of energy is the first law of thermodynamics.  As in Eq. \ref{mass_con}, the $\dqi$ term in Eq. \ref{eq:first_law} consists of all sources of heat such as the fire's heat release rate, convective losses to walls, and radiation exchange {\it etc}. 

The layer temperature, mass and volume are related to the compartment pressure by the ideal gas law:
\begin{equation}
  P\,\Vi = \mi\,R \, \Ti \label{EoS}
\end{equation}

A system of ordinary differential equations for compartment pressure, upper layer volume and upper and lower layer temperature are derived in Appendix \ref{chap:derivation}
using the conservation of mass and energy (Eqs. \ref{mass_con} and \ref{eq:first_law}) and are given below:
...
The mass flow, $\dmau$ and $\dmal$, and heat flow, $\dqu$ and $\dql$, terms are simply the net sum of all mass and heat flowing into a layer (through vents, from fires, between layers via a plume, or by radiation and convection from compartment surfaces).

Algebraic equations for wall surface temperatures can be derived using Fourier's law
\be
   \dq'' = -k \, \frac{d\Tw}{dx}  \label{eq:fourier}
\ee

\noindent so that the temperature gradient, $d\Tw/dx$, at the wall surface is consistent with the heat flux, $\dq''$,
...
for $P$, $\Vu$, $\Tu$, $\Tl$ and $\Tw$ (surface wall temperature) 
...
\begin{eqnarray}
   \cv\ddt\left( \ml \Tl\right)  &=  &\dql - P \dbydt{\Vl}\label{eq:lower}\\
   \cv\ddt\left( \mmu \Tu\right)  &=  &\dqu -  P \dbydt{\Vu}\label{eq:upper}
\end{eqnarray}

\noindent Summing Eqs. \ref{eq:lower} and \ref{eq:upper}, after noting that $\dbydt{\Vl}=-\dbydt{\Vu}$ since $\ddt(\Vl+\Vu)=dV/dt=0$, gives

\begin{equation}
   \cv\ddt\left( \ml \Tl+\mmu \Tu\right)  =  \dql + \dqu
\end{equation}
...
\begin{eqnarray}
   \frac{\cv V}{R}\dbydt{ P}  &=  &\dql + \dqu
\end{eqnarray}

\noindent after assuming that $\ml \Tl/\Vl=\mmu \Tu/\Vu = P/R$ (product of temperature and density for upper and lower layer assumed to be the same).
Finally $R/\cv=\gamma-1$ since $R=\cp-\cv$ and $\gamma=\cp/\cv$ giving
...
To derive the upper layer volume differential equation, first note from Eq. \ref{eq:ideal_gas_law} and the definition of $\gamma$ that
\begin{equation}
\cv\mmu\Tu=\cv\Vu\frac{P}{R}=\frac{\cv}{\cp-\cv}\Vu P=\frac{1}{\gamma-1}\Vu P
\label{eq:vua}
\end{equation}
\noindent Substituting Eq. \ref{eq:vua} into the upper layer form of Eq. \ref{eq:afirst_law} (${\rm i}={\rm u}$) gives
\begin{eqnarray}
\dqu&=&\frac{1}{\gamma-1}\ddt\left(\Vu P\right) + P\dbydt{\Vu}\\
&=&\frac{1}{\gamma-1}\left(P\dbydt{\Vu} + \Vu\dbydt{P}\right) + P\dbydt{\Vu}\\
...
\begin{equation}
\ddt(\cv\mmu\Tu)=\cv\dmu\Tu+\cv \mmu\dbydt{\Tu}
\end{equation}
...
\begin{eqnarray}
   \ddt\left(\cv \mmu \Tu\right) +  P \dbydt{\Vu} &=  &\dqu\\
\cv\dmu\Tu+\cv \mmu\dbydt{\Tu}+\frac{\gamma-1}{\gamma} \dqu - \frac{\Vu}{\gamma}\dbydt{P}&=&\dqu
\label{eq:tua}
\end{eqnarray}
