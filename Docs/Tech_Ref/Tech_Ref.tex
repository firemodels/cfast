...
$\vecy$ is the solution vector containing the quantities $P$, $\Tu$, $\Vu$,  $\Tl$ and $T_w$  at time $t$ and $\vecy'$ is the derivative of these quantities at time $t$.
...
Wall surface temperatures are updated in time implicity using the solver DASSL which couples wall surface temperatures with gas solution variables (pressures, layer volulumes and temperatures).   Target temperatures on the other hand are updated in time using a semi-implicit time marching scheme.  Since they do not affect compartment conditions significantly, there is no need to couple them with other solution variables as is done with wall temperatures.
...
where $k$, $\rho$ and $c$ are the thermal conductivity, density and heat capacity of the target or wall material. The boundary condition used at the wall surface is :
\be
   \Tw(0,t)=T_w
\ee
