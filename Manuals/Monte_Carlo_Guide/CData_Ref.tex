...
In fire safety in the 80’s and before research was ongoing to move toward performance-based designs and away from more prescriptive designs that were then the current state of the art. Bukowski \cite{Bukowski_1985} suggested using the Monte Carlo method as a means of demonstrating alternative designs that were as safe or safer than code compliant designs. The basic idea was to take a design made to meet code and compare its fire safety performance to an alternative design using a set of scenarios with a range of fires. If the relative performance of the alternative design was as good or better then the code compliant design that would be justification for having the alternative design approved. Clarke et al. \cite{Clarke_1990}, as part of demonstrating the viability of using a computer model to predict fire safety, used a Monte Carlo analysis using the Consolidated Fire And Smoke Transport (CFAST) model to predict fire statistics. The process continued and now building codes \cite{NFPA_5000} and engineering handbooks \cite{Hurley_2016} provide a legal and technical structure for a fire hazard analysis using, in part, Monte Carlo methods to characterize the relative fire hazard performance of designs. 

%
% Take the introductory sentence and the Kathy stuff and really the rest as a first draft just to make clear what I was thinking. % Feel free to change it all. 
%
Work continued to better characterize the Monte Carlo method in its use in fire safety research and design. Kathy blah blah blah using Monte Carlo methods to characterize and understand uncertainty in performance-based designs. Bruns \cite{bruns_tn_2016}, in a study using Monte Carlo methods to assess the impact on inner liners in residential upholstered furniture, formalized the mathematics for applying the Monte Carlo method to fire hazard analysis as a means to further incorporate the method in regular performance-based designs. 

Taking a step back, the Monte Carlo method is "a broad class of computational algorithms that use repeated random sampling to obtain numerical results." \cite{wikipedia_montecarlo} Stanislaw Ulam is credited with inventing the modern version of the Markov Chain Monte Carlo method working on nuclear weapons projects in the 1940s. \cite{Metropolis:1987} However, one of the limiting factors in a more widespread adoption of Monte Carlo analysis in fire safety designs has been the large amount of data that needs to be generated and processed.
