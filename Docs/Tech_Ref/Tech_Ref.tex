...
\ee
where $\epsilon_{\rm t}$ is the emissivity of the target, $\theta$ is the angle formed by the line of sight segment of length $R$ and the normal direction to the target surface, $F_j$ is a view factor for the walls, upper layer, or lower layer, $\tau_j$ is a value of transmittance for the path between the target and the walls, upper layer, or lower layer, and $T_{\rm j}$ and $T_{\rm t}$ are the temperature of the gas layers and target.
