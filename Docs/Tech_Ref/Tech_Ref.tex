...
The compartment lining is divided into four parts: the ceiling, the floor, and the wall sections above and below the layer interface. The difference in outgoing and incoming radiative heat flux at a surface~$k$ or net radiative flux,
$\Delta \dot{q}_{{\rm r},k}''=\dot{q}_{{\rm out},k}-\dot{q}_{{\rm in},k}$, is found by solving the simplified radiation transport equation given by Eq.~(17-20) in Siegel and Howell~\cite{SiegelandHowell:1981}:
\be
   \frac{\Delta \dot{q}_{{\rm r},k}''}{\epsilon_k} - \displaystyle\sum_{j=1}^N \frac{1 - \epsilon_j}{\epsilon_j} \;  F_{k-j} \; \tau_{k-j} \; \Delta \dot{q}_{{\rm r},j}'' = \sigma T_k^4 - \displaystyle\sum_{j=1}^N \brackets{F_{k-j} \; \tau_{k-j} \; \sigma T_j^4 \, } - c_k \label{RTE}
\ee
