...
by Andersson and Van Hees \cite{Andersson:2005} 
...
\label{eq:cylheat2}
\end{equation}
with $\dot{q}_{ext}''A_N$ specified as a boundary condition, 
...
=
\left[
\left(\frac{T_{i+1}-T_i}{\Dr}\right)A_i-\left(\frac{T_{i}-T_{i-1}}{\Dr}\right)A_{i-1}
\right]
\label{eq:cylheat2a}
\end{equation}
...
\noindent Defining $C_i$ and $D_i$ as
\begin{eqnarray*}
...
\noindent The energy balance for the $N$'th control volume may be obtained
by substituting $\Dr q''_{ext}/k$ for $\Tipo-\Ti$ in
equation (\ref{eq:cylheat6}) and substituting $C_N$ and $D_N$ 
to obtain
...
Equations (\ref{eq:cylheat8}) and (\ref{eq:cylheat10}) represent a
 tri-diagonal system of equations which when written in matrix form
are given by 
\begin{equation}
\left(
