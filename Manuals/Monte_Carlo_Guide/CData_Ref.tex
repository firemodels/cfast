...
\begin{description}
  \item[CASES] (default value 1): specifies the number of cases that the preprocessor will generate.
...
The {\ct \&MRND} inputs specify distributions for variation of one or more input parameters that are varied for the different CFAST cases to be generated by the preprocessor.
...
\begin{description}
  ...
  \item[DISTRIBUTION] identifies the type of distribution. Additional required inputs depend of the type of distribution defined. Allowed distributions are {\ct UNIFORM}, {\ct TRIANGLE}, {\ct NORMAL}, {\ct LOG\_NORMAL}, {\ct BETA}, and {\ct USER\_DEFINED}.
  \item[SEEDS] like {\ct SEEDS} in {\ct \&MHDR}, an integer pair specifies initial seeds for the random number generator, but applies only to the specific distribution. Any two integers (excluding -1001 which is used internally to indicate default values) may be specified. Including random seeds here will ensure that the same cases will be generated each time the preprocessor is run for a given input file. Changing the random seeds (or other inputs) will result in a different set of input files. All seeds are written in the {\ct <project>\_seeds.csv} file, if specified.
  ...
  \item[MINIMUM] minimum value for the {\ct UNIFORM} and {\ct TRIANGLE} distribution types.
  \item[MAXIMUM] maximum value for the {\ct UNIFORM} and {\ct TRIANGLE} distribution types.
