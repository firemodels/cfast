...
\item[Smoke detectors:] Smoke detectors follow a statistical smoke alarm activation model developed for upholstered furniture containing polyurethane foam \cite{Cleary:2017}. For this simple example,we use only one detector who activation varies with a log-normal distribution with a geometric mean of 3.0~\%/ft $\pm$ 1.3 \%/ft for flaming fires and 5.0~\%/ft $\pm$ 1.3 \%/ft for smoldering fires.
\end{description}
...
\begin{lstlisting}
&MHDR CASES = 10000 /
&MHDR CASES = 20000 WRITE_SEEDS = .TRUE.
...
\begin{description}
  ...
  \item[DISTRIBUTION] identifies the type of distribution. Additional required inputs depend of the type of distribution defined. Allowed distributions are {\ct UNIFORM}, {\ct TRIANGLE}, {\ct NORMAL}, {\ct LOG\_NORMAL}, {\ct BETA}, {\ct LINEAR }, and {\ct USER\_DEFINED}.
  ...
  \item[MINIMUM\_ID] the minimum or maximum value of the distribution can be defined by another input (for example, the top of a door must be less than the compartment height). Define the lower limit with {\ct MINIMUM\_ID} and {\ct MINIMUM\_FIELD} inputs. {\ct MINIMUM\_ID} specifies the name of the specific input (i.e., the name of a compartment or vent).
  \item[MINIMUM\_FIELD] the minimum or maximum value of the distribution can be defined by another input (for example, the top of a door must be less than the compartment height). Define the lower limit with {\ct MINIMUM\_ID} and {\ct MINIMUM\_FIELD} inputs. {\ct MINIMUM\_FIELD} chooses the specific input for the chosen object (for example: {\ct TOP} or {\ct BOTTOM} for a vent).
  \item[MAXIMUM\_ID] the minimum or maximum value of the distribution can be defined by another input (for example, the top of a door must be less than the compartment height). Define the upper limit with {\ct MAXIMUM\_ID} and {\ct MAXIMUM\_FIELD} inputs. {\ct MAXIMUM\_ID} specifies the name of the specific input (i.e., the name of a compartment or vent).
  \item[MAXIMUM\_FIELD] the minimum or maximum value of the distribution can be defined by another input (for example, the top of a door must be less than the compartment height). Define the upper limit with {\ct MAXIMUM\_ID} and {\ct MAXIMUM\_FIELD} inputs. {\ct MAXIMUM\_FIELD} chooses the specific input for the chosen object (for example: {\ct TOP} or {\ct BOTTOM} for a vent).
\end{description}

\vspace{\baselineskip}
\noindent Example:
\begin{lstlisting}
&MRND ID = 'Example rand generator', DISTRIBUTION = 'UNIFORM',  MINIMUM = 10, MAXIMUM = 50 /
&MRND ID ='Second rand generator', DISTRIBUTION = 'NORMAL', MEAN = 0, STDEV = 1 /
\end{lstlisting}

\subsection{Namelist MFLD}

\begin{description}
  \item[ID] Input fields are defined by a unique alphanumeric name. This may be as simple as a single character or number, or a description of the field. All IDs must be unique throughout an input file.
  \item[FYI] A user defined comment that can provide additional information about the input.
  \item[FIELD\_TYPE] specifies the type of field data used to fill in values in the specified field. Additional inputs depend on the type of field specified. Allowed field types are {\ct VALUE}, {\ct SCALING}, {\ct LABEL}, and {\ct INDEX}. {\ct VALUE} simply takes the value returned from the specified random generator and places it directly in the specified field. {\ct SCALING} takes the returned value and multyplies by the {\ct BASE\_SCALING\_VALUE} and puts that value in the field. {\ct INDEX} requires the random generator to return integers starting at 1 to {\ct NUMBER\_IN\_INDEX}. The value in the {\ct INDEX\_TYPE}, {\ct REAL}, {\ct INTEGER}, {\ct CHARACTER}, or {\ct LOGICAL}, array at the index is placed in the field. {\ct LABEL} {\ct FIELD\_TYPE} is primarily meant to be used with other fields being {\ct INDEX} type. It allows you to have a single colum that says which secnario or index the random generator returns. For example suppose you have thre patterns for the initial openings of the vents and when they change if they do. You probably need all the variaous conditions but you might also want a single collumn that tells you which case or scenario you are in. Suppose that you set the doors to open representing fire fighters arriving at 4 min, 6 min and 8 min. 
  \item[RAND\_ID] specifies the ID of associated {\ct \&MRND} input used to provide random inputs for the field. These may be unique to each filed input or more than one field input may use the same {\ct \&MRND} input to coordinate values for multiple input fields.
  \item[ADD\_TO\_PARAMETERS] set to .TRUE. to include the value of the field in the parameters file, {\ct <project>\_parameters.csv}.
  \item[SCENARIO\_TITLE\_ARRAY] The purpose of this is to work with {\ct INDEX}. Its function could just be moved to {\ct STRING\_VALUES}
  \item[PARAMETER\_HEADER] specifies the column title for this input filed in the parameters file, {\ct <project>\_parameters.csv}.
  \item[INTEGER\_VALUES] a set of integer values for the {\ct INDEX} field type. Values for an individual scenario are chosen randomly from the list of values provided.
  \item[LOGICAL\_VALUES]a set of logical values for the {\ct INDEX} field type. Values for an individual scenario are chosen randomly from the list of values provided.
  \item[REAL\_VALUES] a set of real values for the {\ct INDEX} field type. Values for an individual scenario are chosen randomly from the list of values provided.
  \item[STRING\_VALUES] a set of string values for the {\ct INDEX} field type. Values for an individual scenario are chosen randomly from the list of values provided.
  \item[INDEX\_TYPE] This seems superfluous ... Can't you just tell from which one does not have default values? These are needed because of including logical values and there being no clear none value
  \item[NUMBER\_IN\_INDEX] Again, can't you just tell by how many values are input? Same as above. 
  ...
  \item[OBJECT\_ID] specifies the name of the specific input (i.e., the name of a compartment or vent).
  \item[FIELD\_NAME] specifies the specific field in the input {\ct OBJECT\_ID}.
\end{description}

\vspace{\baselineskip}
\noindent Example:
\begin{lstlisting}
&MFLD ID = 'Height_bedroom' OBJECT_ID = 'Bedroom' FIELD_NAME = 'HEIGHT' RAND_ID = 'uniform_generator' PARAMETER_HEADER = 'Bedroom height' ADD_TO_PARAMETERS = .TRUE. /
\end{lstlisting}
