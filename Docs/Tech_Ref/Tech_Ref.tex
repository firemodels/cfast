...
CFAST uses the differential~/~algebraic solver DASSL~\cite{DASSL:1982,DASSL:1989} to solve the differential equations Eqs. (\ref{eq1}) through (\ref{eq4}) and the algebraic equation (once it is discretized) Eq. (\ref{bc}) for $P$, $\Vu$, $\Tu$, $\Tl$ and $\Tw$ (at the surface only).
DASSL solves these equations by formulating them as a root finding problem, $F\brackets{t,\vec{y},\vec{y}'}=0$, where $t$ is the time,
$\vec{y}$ is the solution vector and $\vec{y}'$ is the solution vector derivative at time $t$.

The solution vector, $\vec{y}$  is defined as follows
\begin{center}
\begin{tabular}{r l l}
...
The solution vector derivative, $\vec{y}'$ is defined analogously.

In the simplest case, all temperatures are initialized to ambient, all (over) pressures are initialized to zero, and the derivative components of$\vec{y}'$ are zero. For more complex cases, an initial $\vec{y}$ and $\vec{y}'$ at $t=0$ is calculated and supplied to DASSL.

The function $F$ is arranged analogously to $\vec{y}$ and $\vec{y}'$. The first $n$ components correspond to the $dP/dt$ differential equations, the next $n$ to the $d\Vu/dt$ differential equations and so on.  Each $F$ component is a residual which DASSL drives to zero during the solution process.  For example if $dP_1/dt$ (differential equation for pressure in the first compartment) is computed using Eq. (\ref{eq1}), then  $F_1(t,\vec{y},\vec{y}')$ (first component of $F$) is given by the difference of $y'_1$ (estimate provided by DASSL) and the $dP/dt$ term computed from Eq. (\ref{eq1}) using $\vec{y}$ or equivalently

\begin{equation}
F_1(t,\vec{y},\vec{y}')=\vec{y}'_1 - dP_1/dt
\label{eq:Fp1}
\end{equation}
DASSL then determines values for $\vec{y}$ and $\vec{y}'$ so that Eq. (\ref{eq:Fp1}) along with all other components of $F$ sufficiently close to zero (according to a user defined error criteria).
\renewcommand{\arraystretch}{1.5}
A definition of $F$ for all components is given by
\begin{equation}
F_i(t,\vec{y},\vec{y}')=\left\{
\begin{array}{ll}
  y'_i-\dbydt{P} & 1\le i \le n \\
  y'_i-\dbydt{\Vu} & n+1\le i \le 2n \\
  y'_i-\dbydt{\Tu} & 2n+1\le i \le 3 n \\
  y'_i-\dbydt{\Tl} & 3n+1\le i \le 4n \\
  q''(\vec{y})+\dbydt{\Tw}(0,t) & 4n+1\le i \le 4n +k 
\end{array}
...
One step of the solution process then is to advance the solution from $t_n$ to $t_{n+1}$ {\em i.e.}\ to find the solution of $F(t_{n+1},\vec{y}_{n+1},\vec{y}'_{n+1})=0$ given that the solution for
$F(t_{n},\vec{y}_{n},\vec{y}'_{n})=0$ is known.
The solver estimates values for $\vec{y}$ and $\vec{y}'$ at $t+\Delta t$. CFAST does this by calculating an actual $\vec{y}'$ from the estimated values of $\vec{y}$ and returns the difference between the estimated $\vec{y}'$ and calculated $\vec{y}'$ to DASSL using Eq. (\ref{eq:resid}). DASSL then advances the solution using an extension of Newton's method using derivatives that are approximated with backward differentiation formulae of orders one through five to solve for $\vec{y}$ and $\vec{y}'$ to within a specified tolerance~\cite{DASSL:1982,DASSL:1989}.
...
\begin{enumerate}
\item Assume that $\vec{y}_n$ and $\vec{y}'_n$ are known at $t=t_n$
\item For some $\Delta t$ guess values for $\vec{y}_{n+1}$ and $\vec{y}'_{n+1}$ at $t_{n+1}=t+\Delta t$
\item Using the guessed solution, compute heat and mass flow terms and a new temperature profile $t=t_{n+1}$
\end{enumerate}
