...
DASSL solves these equations by putting them into the form of a root finding problem, $F\brackets{t,\vec{y},\vec{y}'}=0$, where $t$ is the time, 
$\vec{y}$ is the solution vector and $\vec{y}'$ is the solution vector derivative at time $t$. 

The solution vector, $\vec{y}$  is
\begin{center}
\begin{tabular}{r l l}
...
The vector, $\vec{y}'$ is defined analogously.  
...
The function $F$ is arranged analogously to $\vec{y}$ and $\vec{y}'$. The first $n$ components correspond to the $dP/dt$ differential equations, the next $n$ to the $d\Vu/dt$ differential equations and so on.  Each component of $F$ is a residual which DASSL drives to zero during the solution process.  For example if $dP_1/dt$ (differential equation for pressure in the first compartment) is computed using Eq. (\ref{eq1}), then $F_1(t,y,y')$ is given by the difference $\vec{y}'_1$ (estimated provided by DASSL) and the $dP/dt$ term computed from Eq. (\ref{eq1}) and DASSL supplied estimates for $\vec{y}$   or equivalently

\begin{equation}
F_1(t,\vec{y},\vec{y}')=\vec{y}'_1 - dP_1/dt
\end{equation}

A more complete definition of $F$ is given by
...
At each time step, the solver estimates values for $\vec{y}$ and $\vec{y}'$ at $t+\Delta t$, CFAST calculates an actual $\vec{y}'$ from the estimated values of $\vec{y}$ and returns the difference between the estimated $\vec{y}'$ and calculated $\vec{y}'$ to DASSL. DASSL uses an extension of Newton's method to approximate the derivatives using backward differentiation formulae of orders one through five to solve for $\vec{y}$ and $\vec{y}'$ to within a specified tolerance~\cite{DASSL:1982,DASSL:1989}.
