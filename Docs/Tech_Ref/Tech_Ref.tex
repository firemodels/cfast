...
\noindent A differential equation for compartment pressure may be derived by using Eq. \ref{eq:first_law} for both layers, the ideal gas law approximation $P/R\approx\rho_l T_l\approx\rho_u T_u$ and standard
 definitions for density, $R$ and $\gamma$.  Rewriting Eq. \ref{eq:first_law} for the upper and lower layer and substituting $\rho_i=m_i/V_i$ gives
\begin{eqnarray}
   c_v\dbydt{( V_l\rho_l T_l)} +  P \dbydt{V_l} &=  &\dot h_l \label{eq:lower}\\
   c_v\dbydt{( V_u\rho_u T_u)} +  P \dbydt{V_u} &=  &\dot h_u \label{eq:upper}
\end{eqnarray}

\noindent Summing Eqs. \ref{eq:lower} and \ref{eq:upper} using $dV_l/dt=-dV_u/dt$ since $d/dt(V_l+V_u)=dV/dt=0$ and using the assumption that $\rho_l T_L=\rho_u T_U = P/R$ gives
\begin{eqnarray}
   \frac{c_vV}{R}\dbydt{ P}  &=  &\dot h_l + \dot h_u
\end{eqnarray}

\noindent Finally using $R=c_p-c_v$ and $\gamma=c_p/c_v$ to show that $R/c_v=\gamma-1$ gives
