...
C***FIRST EXECUTABLE STATEMENT  XERROR
      ...
      ENDIF
      NMESS = MAX(1,NMESS)
!      IF(LOGERR>0 .AND. LEVEL/=0) WRITE(LOGERR,*) MESSG(1:NMESS)
      IF(LEVEL/=-2)THEN
...
C***PURPOSE  Returns real*8 machine dependent constants
C***DESCRIPTION
...
C
      ...
      IP1=I+1
      DO 10 J=IP1,NRET
      IF (PRSNID(I)*10+HAZARD(I)>PRSNID(J)*10+HAZARD(J)) THEN
        IT=PRSNID(I)
        PRSNID(I)=PRSNID(J)
        PRSNID(J)=IT
        IT=HAZARD(I)
...
C   PUT IN COMMAS WHERE APPROPRIATE TO DELIMIT ALL FIELDS
   ...
   30   IF (CMDLIN(IC:IC)==' ') THEN
          IF (CMDLIN(IC-1:IC-1)/=','.AND.CMDLIN(IC+1:IC+1)/=',') 
     +        THEN
            CMDLIN(IC:IC) = ','
            IC = IC + 1
          ELSE
        ...
        ELSE IF ((CMDLIN(IC:IC)==OPTSEP).AND.CMDLIN(IC-1:IC-1)/=',')
     +      THEN
...
C
      ELSE IF (CHR=='E'.OR.CHR=='e'.OR.CHR=='D'.OR.CHR=='d'
     +    .OR.CHR=='+'.OR.CHR=='-') THEN
        IESGN = 1
        IF (CHR=='-') IESGN = -1
      ELSE
