...
A target is located at the center of a 4~m by 4~m by 4~m enclosure. Assuming adiabatic condition for the target, an adiabatic surface temperature\footnote{CFAST has an option to create a target with adiabatic surface conditions. The option must be invoked by adding a special comment to the input file.} can be evaluated from solving the following energy balance equation:
\be
