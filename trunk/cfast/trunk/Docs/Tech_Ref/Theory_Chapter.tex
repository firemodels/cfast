...
&=&C\sqrt{2\rho}w\frac{2}{3}\frac{t-b}{|P_t|-|P_b|}(|P_t|^{3/2}-|P_b|^{3/2})
...
Equation \ref{eq:massflowone} can be simplified 

\be \dm_{io} = \frac{2}{3} C \sqrt{2 \rho} A_{slab} \brackets{\frac{x^2+xy+y2}{x+y}} \ee

where $x = \sqrt{|P_t|}$ and $y = \sqrt{|P_b|}$ and $A_{slab}=w(t-b)$. $P_t$ and $P_b$ are the cross-vent pressure differential at the top and bottom of the slab respectively and $A_{slab}$ is the cross-sectional area of the slab.  The value of the density, $\rho$, is taken from the source compartment.
...
{\bf Computing the Transient Target Temperature:}
