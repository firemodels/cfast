...
{\ct SIMULATION}        & Integer   & Section \ref{info:TIME}                 & s         & 900             \\ \hline
...
\endhead
\parbox{1.5in}{\bf Parameter}    & \parbox{1in}{\bf Type}  & \parbox{1in}{\bf Reference}  & \parbox{1in}{\bf Units}  & \parbox{1in}{\bf Default Value} \\ \hline
{\ct CEILING\_MATL\_ID}*\footnote{ * indicates a required input for each {\ct COMP} input included in the input file. At least on {\ct COMP} input must be included in an input file.}     & Character		 & Section \ref{info:COMP2}               &           &                 \\ \hline
...
{\ct ID}*                    & Character		 & Section \ref{info:COMP}                &           & (0,0,0)         \\ \hline
...
{\ct AREAS(1:2)}\footnote{Appropriate for mechanical vent}       & Real Doublet 	          & Section \ref{info:VENT3}     & m$^2$                    &                 \\ \hline
...
{\ct CARBON}               & Real     & Section \ref{info:FIRE}                 &                             &                 \\ \hline
{\ct CHLORINE}             & Real     & Section \ref{info:FIRE}                 &                             &                 \\ \hline
...
\endhead
\parbox{1.5in}{\bf Parameter}    & \parbox{1in}{\bf Type}  & \parbox{1in}{\bf Reference}  & \parbox{1in}{\bf Units}  & \parbox{1in}{\bf Default Value} \\ \hline
{\ct F}*\footnote{ * indicates a required input for each {\ct RAMP} input included in the input file. Either {\ct T} or {\ct Z} will be included depending on the type associated with the ramp.}               & Real Array            & Section \ref{info:RAMP2}                 &          &                 \\ \hline
...
\graybox{
Vent opening is specified as a function of time with {\ct T} and {\ct F} as the fraction of the vent width (for wall vents) or of the vent area (for ceiling, floor, and mechanical vents). Refer to Section~\ref{info:VENT2}. \\

Filter efficiency is specified as a function of time with {\ct T} and {\ct F} as the fraction of soot and trace species removed in the associated mechanical vent. Refer to Section~\ref{info:VENT4}. \\
...
Heat release rate of a fire is specified as a function of time with {\ct T} and {\ct F} as the heat release rate of the fire in kW. Refer to Section~\ref{info:FIRE2}.

Fire species yields are specified as a function of time using {\ct T} as time {\ct F} as the yield fraction in kg of species produced per kg of fuel burned.
}
