...
\begin{description}
\item[Interior Temperature] (default: 20 \degc; namelist: \ct{INTERIOR_TEMPERATURE}): Initial temperature inside the structure at the station elevation.
...
\item[Exterior Temperature] (default: 20 \degc; namelist: \ct{EXTERIOR_TEMPERATURE}): Initial temperature outside the structure at the station elevation.
...
\item[O$_2$ mass fraction] (default: 0.23 kg/kg; namelist: \ct{INTERIOR_O2_MASS_FRACTION} and \ct{EXTERIOR_O2_MASS_FRACTION}):
...
\item[Extinction Coefficient] (default: 8700 (flaming), 4400 m$^2$/kg (smoldering); namelist: \ct{SPECIFIC_EXTINCTION(2)}) Specific-extinction coefficients for flaming and smoldering combustion, respectively.
\end{description}
...
\begin{description}
\item[Visualization Type] (default value: 2-D): The type of visualization to be included. The CEdit Output tab offers 2-D (a single plane slice of temperature at the position and axis specified) and 3-D (a set of three animated slices whose positions can be moved along their respective axes). See the Smokeview documentation \cite{Smokeview_Users_Guide_6} for details on the use of Smokeview.
...
Temperature isosurfaces are not configured in the CEdit Output tab. To request a surface of constant gas temperature, specify an \ct{ISOF} record in the input file:
\begin{description}
