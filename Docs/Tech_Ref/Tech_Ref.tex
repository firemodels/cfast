...
The configuration factor, $F_{1-2}$, is the fraction of radiant energy emitted by surface 1 that is intercepted by  surface 2, and is defined by
\be
...
For a compartment with four surfaces: ceiling, upper/lower wall and floor, 16 configuration factor computations are required. Only three configuration factors require the use of the analytical formula for configuration factors between parallel plates given by
...
\end{eqnarray}
where $a$ and $b$ are the rectangle dimensions, $c$ is the separation distance, $X=a/c$ and $Y=b/c$. This formula is found in Appendix~C of Ref.~\cite{SiegelandHowell:1981}.  The other 13 configuration factors may be computed using simple algebraic formulas derived using symmetry properties such as $A_1F_{1-2}=A_2F_{2-1}$ and $F_{i1}+F_{i2}+F_{i3}+F_{i4}=1$.
...
\be
   \dq_{\rm r}'' = \epsilon_{\rm t} \left( \cos \, \theta \; \frac{\chi_{\rm r} \, \dQ}{4 \pi R^2}  + \displaystyle\sum_{j=1}^{10} \left( \tau_j \, F_j \, \epsilon_j \, q''_{{\rm out},j} \right) +  \displaystyle\sum_{j=1}^2 \left( \tau_j \, F_j \, \epsilon_j \sigma \, T_j^4 \right)  -\sigma T_{\rm t}^4  \right)  \label{target_rad}
\ee
where $\epsilon_{\rm t}$ is the emissivity of the target, $\epsilon_{\rm j}$ is the emissivity of the wall surface, $\theta$ is the angle formed by the line of sight segment of length $R$ and the normal direction to the target surface, $F_j$ and $F_{\rm u}$ are view factors for the walls and upper layer, $\tau_j$ and $\tau_{\rm u}$ are the values of transmittance for the path between the target and the walls and upper layer and $T_{\rm t}$ is the temperature of the target.  The term $q''_{{\rm out},j}=\sigma T_j^4-\frac{1-\epsilon_j}{\epsilon_j}\dq_{{\rm r},j}''$ is the radiative flux leaving the $j$'th wall surface (See Eq.~(17-15) in Siegel and Howell~\cite{SiegelandHowell:1981})\ where $\dq_{{\rm r},j}''$ is the net radiative flux found using Eq.~(\ref{RTE}).
