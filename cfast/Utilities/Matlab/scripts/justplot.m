% R. McDermott and C. Cruz and S. Hostikka, modified by R. Peacock for just
% plotting graphs without any statistics
% 6-11-2014
% justplot.m
%
...
function [saved_data,drange] = justplot(varargin)
...
for i=2:n_plots
    ...
    if itest && (dtest || otest)
        ...
        % Plot the FDS or model data (d2)if there is data
        if ~strcmp(d2_Filename,[Working_Dir,'-1'])
        ...
        try
            ...
            if size(Key_Position)>0
                if ~strcmp(d2_Filename,[Working_Dir,'-1'])
                    ...
                    legend_handle = legend(K,parse(d1_Key),'Location',Key_Position);
                end
