...
For this verification case, the upper layer thickness for each compartment is set to be 1~m and the vent opening fraction for all vents is set to be 0, 0.5, and 1 at  0~sec, 0.5~sec, and 1~sec, respectively. The analytical solutions and CFAST results associated with Compartment 1 (upper and lower layers), Compartment 2, and Compartment 3 are shown in Figures~\ref{SOF_Result_1}, \ref{SOF_Result_2}, \ref{SOF_Result_3}, and \ref{SOF_Result_4}, respectively. Note that vent opening fraction is different than surface opening fraction.
...
Air is supplied to a single compartment at ambient conditions via a single mechanical vent with a flowrate of 0.01 m$^3$/s. Wall or floor leakage is specified in two test cases. Flow out the leak should equal the specified flow in through the mechanical vent.  The expected mass flow rate is just the specified volumetric flow times the density of air at ambient conditions for an expecte value of 0.012 kg/s. The expected results and CFAST results for the wall leak and floor leaks are shown in Figures~\ref{Leak_Result_1} and \ref{Leak_Result_2}, respectively.

\begin{figure}[!ht]
\centering
\includegraphics[width=3.0in]{SCRIPT_FIGURES/Verification/Leakage_1}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=3.0in]{SCRIPT_FIGURES/Verification/Leakage_2}
\caption[Results of the test case {\ct Leakage\_2.in}]{Expected and CFAST calculated floor leakage for the case {\ct Leakage\_2.in}.}
\label{Leak_Result_2}
\end{figure}
