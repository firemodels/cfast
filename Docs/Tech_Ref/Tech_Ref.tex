...
the spatial derivative term $\dbydx{\Tw}$ and is evaluated by advancing the wall temperature profile from a previously known solution at $t_{\rm old}$ to $t$ using Eq. (\ref{eq:fourier}) and the temperature boundary condition $\dbydx{\Tw}(0,t)=T_{\rm w}$ provided by DASSL in $\vecy$.
...
\label{step:residode}
\item Compute the temperature gradient residual (last row of Eq. (\ref{eq:resid})):
