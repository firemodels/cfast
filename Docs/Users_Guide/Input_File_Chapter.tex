...
\item[Bottom Compartment] The bottom or second of the two compartments to be connected by a ceiling vent.
...
\item[TS Yield] (default units: kg/kg, default values: none): Yield of user-defined trace species per mass of fuel produced in the pyrolysis of the fuel.
...
For a fire, the heat release rate is limited by combustion efficiency and available oxygen. In CFAST, the combustion efficiency is assumed to be 100~\% so that in essence, the input for heat release rate is a net heat release rate accounting for any inefficiency for the burning fuel. The limit for available oxygen is applied in three places: The first is to limit burning in the portion of the plume which is in the lower layer of the room of fire origin based on the oxygen entrained in the plume; the second is the portion of the plume in the upper layer, also in the room of origin; the third is in the vent flow which entrains air from a lower layer into an upper layer in an adjacent compartment. The unburned fuel is tracked in this model. Combustion of CO to CO$_2$ is not included in the model. Detailed combustion chemistry is not considered in CFAST due to the complexities associated with detailed kinetics and transport. \\
...
\subsection{Editing Fires}

By default, CEdit does not include predefined fires. Thus, the user needs to define fire objects for use with a specific simulation.  These may be from other simulations or input directly from reference sources or test results. The `From File' button allows you to add fires from an existing data file to the current simulation.
...
\item[Ignition Criterion] The type of ignition condition, which can be time, surface temperature or incident flux to object surface.

\item[Set Point] The numerical value at which ignition will occur. If it is less than or equal to zero, the default value of zero is taken.
\end{description}
...
\item[Width (X)] (default units: m, default value: none): Distance from the left wall of the target compartment (X direction).

\item[Depth (Y)] (default units: m, default value: none): Distance from the front wall of the target compartment (Y direction).

\item[Height (Z)] (default units: m, default value: none): Height of the target above the floor (Z direction).
