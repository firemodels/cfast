...
Time Histories          & \multicolumn{2}{|c|}{See Table \ref{tbl:fire_variables}}   \\ \hline
\end{longtable}
...
{\ct REAL\_VALUES}. The number of {\ct PROBABILITIES} inputs must be one less than then number of {\ct REAL\_VALUES} inputs. Returns real values. Endpoints of each interval are inclusive and interior points are continuous. For example, if you have {\ct REAL\_VALUES} = 1.,3.,8. and {\ct PROBABILITIES} = 0.3,0.7, there’s then a 30\% probability of getting a real number between 1.0 and 3.0 and a 70\% probability of getting a real number between 3.0 and 8.0.
        \item[USER\_DEFINED\_DISCRETE] requires additional inputs {\ct PROBABILITIES}, and \\
{\ct REAL\_VALUES}. The number of {\ct PROBABILITIES} inputs must match the number of \\
{\ct REAL\_VALUES} inputs. Returns real values.
      \end{description}
  \item[RANDOM\_SEEDS] like {\ct RANDOM\_SEEDS} in {\ct \&MHDR}, an integer pair specifies initial seeds for the random number generator, but applies only to the specific distribution. Any two integers (excluding -1001 which is used internally to indicate default values) may be specified. As an example suppose that a set of 1000 cases are created and run. For some reason the user wants to have the next 1000 cases to have the ceiling height in one compartment to be the same as the first run but have other variables change. Setting the {\ct RANDOM\_SEEDS} in the {\ct \&MFLD} that sets the ceiling height in that compartment will result in the ceiling heights being the same while other variable are different. All seeds are written in the {\ct <project>\_seeds.csv} file, if specified.
  \item[REAL\_CONSTANT] is a single real value. For the {\ct REAL\_CONSTANT} distribution it is the value that is returned by the generator.
  \item[REAL\_VALUES] a set of real numbers for the {\ct USER\_DEFINED} distribution types. Inputs are input as real constants. Values for an individual scenario are chosen randomly from the list of values provided.
...
&MRND ID = 'Vent Height Generator' DISTRIBUTION_TYPE = 'UNIFORM' MINIMUM = 1.5 MAXIMUM_FIELD = 'Comp 1 Height' MAXIMUM_OFFSET = -0.5/
...
The first two inputs varies the height of the compartment, a uniform distirbution 2.0~m to 3.0~m. The third input is the CFAST definition of the compartment. The next two inputs define the distribution for the door width, a uniform distribution from 0.25~m to 2.0~m. The third two inputs define the distribution for the height of the door. There is a problem because the compartment's height varies. One solution would be to have the {\ct MAXIMUM} for the {\ct TOP} of the door to be less the {\ct MINIMUM} compartment {\ct HEIGHT}. However, suppose that we always want the space between the top of the door and the ceiling to be at least 0.5~m. One way to do this is use {\ct MAXIMUM\_FIELD} and {\ct MAXIMUM\_OFFSET}. {\ct MAXIMUM\_FIELD} means that for every case being generated the value of the referenced field will be found first and than will be used as the maximum value for this random generator this case. {\ct MAXIMUM\_OFFSET} is a value added to the maximum value to determine the final maximum value, in this case -0.5~m to insure the top of the door is at least 0.5~m below the ceiling. The last input defines the normal CFAST input defining the base values for the door. The {\ct WIDTH} and {\ct HEIGHT} inputs are replaced with random values for each individual CFAST input file generated. All other values in the {\ct \&VENT} input remain at the base values.
