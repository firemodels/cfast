...
\end{equation}
where $\rho$, $c$ and $k$ are the density, specific heat and thermal conductivity of the wall material. Further details of the solution procedure are included Appendix~\ref{chap:heatsolution}.
...
\label{step:residode}
\item Using the updated wall surface temperatures, $T_{{\rm w},i}$, estimated in step~\ref{step:first} as a boundary conditions, advance the temperature profiles, $T_{{\rm s},i}(x,t)$, from $t=t_n$ to $t=t_{n+1}$ using an implicit time update of Eq.~(\ref{eq:fouriera}).
\item Compute the residuals $q_i''+k\dbydx{T_{{\rm s},i}}$ using the heat flows computed in step \ref{step:rhs} and the surface temperature gradients computed in step~\ref{eq:fouriera}.
