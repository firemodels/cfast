...
As discussed in Refs.~\cite{Forney:1994} and \cite{Rehm:1992}, Eqs.~(\ref{eq1}) through (\ref{eq4}) are stiff since the pressure adjusts to changing conditions far more quickly than the other variables. Runge-Kutta methods or predictor-corrector methods such as Adams-Bashforth require prohibitively small time steps in order to track the short time scale phenomena (pressure in our case). Methods that calculate the Jacobian (or at least approximate it) have a much larger stability region for stiff problems and are thus more successful at their solution (see, for example, \cite{Numerical_Recipes}).

The differential equation set, Eqs. \ref{eq1} - \ref{eq4} and algebraic equation set (for compartment surface temperatures, Eq. \ref{eq:net_fluxes}) are solved as a set of simultaneous differential / algebraic equations of the form $F\brackets{t,\vec{y},\vec{y}'}=0$ by the DAE solver, DASSL \cite{DASSL:1982,DASSL:1989}, where $F\brackets{t,\vec{y},\vec{y}'}$ is the functional relationship between $t$ (the current time in the simulation), $\vec{y}$ (the vector of solution variables at time $t$), and $\vec{y}'$ (the vector of first derivatives of the solution variables at time $t$). The vector of solution variables, $\vec{y}$, solved by DASSL is
