...
\be
   ...
   \displaystyle\sum_{{\rm f}=1}^{n_{\rm fires}} \cos(\theta_{\rm f}) \tau_f \; \frac{\chi_{\rm r} \, \dQ_{\rm f}}{4 \pi R_{\rm f}^2}  + \displaystyle\sum_{w=1}^{10} \tau_w \, F_w \, \epsilon_w \, \qout{w}  +  \displaystyle\sum_{g=1}^2 \left(\tau_g \, F_g \, \sigma \, T_g^4 \right)  -\sigma T_{\rm t}^4  \right)  \label{target_rad}
\ee
where $\epsilon_{\rm t}$ is the emissivity of the target, $\epsilon_{\rm j}$ is the emissivity of the wall surface, $\theta_f$ is the angle formed by the line of sight segment (between the fire $f$ and the target) of length $R_f$ and the normal direction to the target surface, $F_w$ and $F_{\rm g}$ are view factors for the walls and gas layers, $\tau_f$, $\tau_w$, and $\tau_{\rm g}$ are the values of transmittance for the path between the target and the fires, walls, and gas layers, $T_{\rm t}$ is the temperature of the target.  The term $\qout{w}$, given by Eq.~(\ref{eq:qoutb}), is the outgoing radiative flux at the $j$'th wall surface.
