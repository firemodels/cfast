...
\end{itemize}
The compartment lining is divided into four parts: the ceiling, the floor, and the wall sections above and below the layer interface. The net radiative heat flux leaving a surface~$k$, $\dq_{{\rm r},k}''$, is found by solving the simplified radiation transport equation given by Eq.~(17-20) in Siegel and Howell~\cite{SiegelandHowell:1981}:
\be
...
Note, the net radiative flux entering a surface or $-\dq_{{\rm r},k}''$ is used to compute conduction within a solid.
...
\label{eq:configrect}
\end{eqnarray}
where $a$ and $b$ are the rectangle dimensions, $c$ is the separation distance, $X=a/c$ and $Y=b/c$. Algebraic relations in terms of $F_{1-4}$, $F_{1-d}$ and $F_{4-d}$ may then be used to determine the 16 required configuration factors.  Two properties are used to derive these relations.  The symmetry relation, $A_iF_{i-j}=A_jF_{j-i}$, follows from Eq. (\ref{eq:config_factor}). Configuration factors for surfaces forming an enclosure also satisfy
\begin{eqnarray}
\label{eq:factorsum}
\sum_{j=1}^4F_{i-j}=1
\end{eqnarray}
...
Equations (\ref{eq:config1}) and (\ref{eq:config2}) can be solved for $F_{1-2}$ and $F_{2-2}$ to obtain
\begin{eqnarray*}
F_{1-2}&=&1-F_{1-d}\\
