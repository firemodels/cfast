...
Sixteen configuration factor are required for a radiation computation in compartments with four surfaces (ceiling, upper/lower wall and floor). To compute these 16 factors, the formula for configuration factors between parallel plates (see Appendix~C of Ref.~\cite{SiegelandHowell:1981}) given by
...
\end{eqnarray}
where $a$ and $b$ are the rectangle dimensions, $c$ is the separation distance, $X=a/c$ and $Y=b/c$ is first used to compute three configuration factors: a configuration factor between the floor and ceiling, between the floor and layer interface and between the ceiling and layer interface. These three factors are  then used to compute the 16 factors using using properties such as $A_1F_{1-2}=A_2F_{2-1}$ and $F_{i1}+F_{i2}+F_{i3}+F_{i4}=1$.
