...
\caption{The net radiative flux, $\Delta \dot{q}_{{\rm r},k}''$, at each wall surface $k$ is determined so that the 
incoming and outgoing radiation balance.}
\end{figure}

The compartment lining is divided into four surfaces: the ceiling, the floor, and the wall sections above and below the layer interface. As illustrated in Figure \ref{radiationwallsetup}, the problem then is to determine the net radiative flux
$\dot{q}_{{\rm r},k}''$ so that incoming and outgoing radiation balances at each of these four surfaces ($k=1,\cdots,4$).
These fluxes are found by solving the simplified radiation transport equation given by Eq.~(17-20) in Siegel and Howell~\cite{SiegelandHowell:1981}:
\be
   \frac{\Delta \dot{q}_{{\rm r},k}''}{\epsilon_k} - \displaystyle\sum_{j=1}^4 \frac{1 - \epsilon_j}{\epsilon_j} \;  F_{k-j} \; \tau_{k-j} \; \Delta \dot{q}_{{\rm r},j}'' = \sigma T_k^4 - \displaystyle\sum_{j=1}^4 \brackets{F_{k-j} \; \tau_{k-j} \; \sigma T_j^4 \, } - c_k \label{RTE}
\ee
...
The net radiative flux entering a surface, given by $\dq_{{\rm r},k}''=-\Delta \dot{q}_{{\rm r},k}''$, along with the convective flux discussed in Section \ref{section:convection}\ is used as a boundary condition for computing heat conduction within the solid walls.
...
\end{eqnarray*}
The $k$'th component of the $n$-vector $e$ is $e_k=\sigma T^4$, $c_k$ is given by Eq.~\ref{ckeq} and the $k$'th component of the solution vector is $x_k=\Delta\dot{q}_{{\rm r},k}''$.  The term $\delta$ is the usual Kronekar delta function with
\begin{eqnarray*}
\delta_{k,j}=\left\{
\begin{array}{cc}
1&k=j\\
...
The solution to the original problem is then given by $\Delta \dot{q}_{{\rm r},k}''=\Delta q_k\epsilon_k$
where $\Delta q_k$ is the solution to the transformed problem.


\subsection{Configuration Factors}

A configuration factor, $F_{i-j}$, is the fraction of radiant energy emitted by surface $i$ that is intercepted by  surface $j$.  Sixteen configuration factor are required for each compartment since a compartment has four wall surfaces, a ceiling, upper wall, lower wall and floor labeled 1, 2, 3 and 4 as illustrated in Figure \ref{configfactorsetup}.
...
\label{eq:configrect}
\end{eqnarray}
where $a$ and $b$ are the rectangle dimensions, $c$ is the separation distance, $X=a/c$ and $Y=b/c$. Algebraic relations in terms of $F_{1-4}$, $F_{1-d}$ and $F_{4-d}$ may then be used to determine the 16 required configuration factors.  Two properties are used to derive these relations.  The symmetry relation, $A_iF_{i-j}=A_jF_{j-i}$, follows from Eq. (\ref{eq:config_factor}). Configuration factors for surfaces forming an enclosure satisfy
\begin{eqnarray}
\label{eq:factorsum}
\sum_{j=1}^4F_{i-j}=1
\end{eqnarray}
