...
The mass ($\dmau$ and $\dmal$) and enthalpy ($\dhu$ and $\dhl$) terms are simply the net sum of all mass and enthalpy flowing into a layer (through vents, from fires, or from between layers via a plume).
...
The differential equation set, Eqs. \ref{eq1} - \ref{eq4} and algebraic equation set (for the mechanical ventilation system and gas species) are solved as a set of simultaneous differential / algebraic equations of the form $g\brackets{t,y,y'}=0$ by the DAE solver, DASSL \cite{DASSL}, where $g\brackets{t,y,y'}$ is the functional relationship between $t$ (the current time in the simulation), $y$ (the vector of solution variables at time $t$), and $y'$ (the vector of first derivatives of the solution variables at time $t$). The vector of solution variables, $y$, solved by DASSL is


\begin{center}
\begin{tabular}{r l l}
$\boldsymbol{y} =$& $(P_1, P_2, ..., P_{n-1}, P_n,$ & (pressures for each compartment)  \\
               ...
               &  $P_{MV_1}, P_{MV_2}, ..., P_{MV_{m-1}}, P_{MV_m}$, & (pressures at each node of the mechanical ventilation system)  \\
               &  $T_{MV_1}, T_{MV_2}, ..., T_{MV_{m-1}}, T_{MV_m}$ & (temperatures at each node of the mechanical ventilation system) \\
\end{tabular}
\end{center}

In the simplest case, all temperatures are initialized to ambient, all pressures are initialized to zero, and $\boldsymbol{y'}$ is zero. For more complex cases, an initial $y$ and $y'$ at $t=0$ is calculated and supplied to DASSL. At each time step, the solver estimates values for $y$ and $y'$ at $t+\Delta t$, CFAST calculates an actual $y'$ from the estimated values of $y$ and returns the difference between the estimated $y'$ and calculated $y'$ to DASSL. DASSL uses backward differentiation formulas of orders one through five to solve for $y$ and $y'$ to within tolerances specified by CFAST \cite{DASSL}.
...
\be
   ...
   \displaystyle\sum_{{\rm f}=1}^{n_{\rm fires}} \cos(\theta_{\rm f}) \; \frac{\chi_{\rm r} \, \dQ_{\rm f}}{4 \pi R_{\rm f}^2}  + \displaystyle\sum_{j=1}^{10} \tau_j \, F_j \, \epsilon_j \, \qout{j}  +  \displaystyle\sum_{j=1}^2 \left(\tau_j \, F_j \, \epsilon_j \sigma \, T_j^4 \right)  -\sigma T_{\rm t}^4  \right)  \label{target_rad}
\ee
