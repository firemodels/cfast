...
\be
   \dq'' +k \, \frac{\partial T_{\rm w}(0,t)}{\partial x}=0   \label{bc}
\ee
where $\partial T_{\rm w}(0,t)/\partial x$ is the temperature gradient at the wall surface, $\dq''$ is the net radiative and convective heat flux from the adjacent gas layer (see Chapter~\ref{chap:heat_transfer} for a description of the heat flux term) and for brevity the temperature wall profiles are referred to collectively as $T_{\rm w}(x,t)$. The spatial coordinate $x$ refers to the depth inside the wall; $x=0$ is the wall surface. The temperature gradient in Eq.~(\ref{bc}) is determined by first updating the temperature profile by solving the heat conduction equation 
\begin{equation}
...
\begin{array}{ll}
  \vecy'-\mathbf{f}(t,\vecy) & \hbox{gas phase} \\
  \dq''(\vecy) +k \, \frac{\partial T_{\rm w}(0,t)}{\partial x} & \hbox{gas/solid interface} \label{eq:resid}
\end{array}
\right.
\end{equation}
$\mathbf{f}$ is the right hand side of Eqs. (\ref{eq1}) through (\ref{eq4}), $t$ is the time, $\vecy$ is the solution vector consisting of the pressure, upper layer volume, upper and lower layer temperature, and the ceiling, upper wall, lower wall, and floor surface temperatures, $T_{\rm w}(0,t)$, of each compartment. $\vecy'$ is the derivative of these quantities with time. The second equation is used to couple the gas with the solid phase by requiring that the convective and radiative heat flux, $\dq''(\vecy)$, equal the temperature gradient at the wall surface, $\partial T_{\rm w} / \partial x$, where $x$ is the one-dimensional spatial coordinate pointing into the solid. The solid phase temperature profile satisfies the PDE
\begin{equation}
   \frac{\partial T_{\rm w}}{\partial t} = \frac{k}{c \, \rho} \frac{\partial^2 T_{\rm w}}{\partial x^2} \label{eq:fouriera}
\end{equation}
with the surface temperature, $T_{\rm w}(0,t)$, provided by DASSL. An outline of the solution procedure is as follows:
...
\label{step:first}
\item Using the estimated solution, $\vecy_{n+1}$, compute mass flow rates, $\dml$ and $\dmu$, and heat flow rates, $\dql$ and $\dqu$, required to compute the right hand sides of Eqs.~(\ref{eq1}) through (\ref{eq4}), $\mathbf{f}(t_{n+1},\vecy_{n+1})$
\label{step:rhs}
...
\label{step:residode}
\item Using the updated wall surface temperatures estimated in step~\ref{step:first} as a boundary condition, advance the temperature profile $T_{\rm w}(x,t)$ from $t=t_n$ to $t=t_{n+1}$ using an implicit time update of Eq.~(\ref{eq:fouriera}).
\item Compute the residual $q''+k\dbydx{T_{\rm w}}$ using the heat flows computed in step \ref{step:rhs} and the surface temperature gradients computed in step~\ref{eq:fouriera}.
\item Check if all the residuals are within a specified error tolerance. If not, reduce the time step and repeat the procedure.
\end{enumerate}
