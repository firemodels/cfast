...
An initial run of 1000 cases was evaluated. The chart below shows the coefficient for \texttt{Lab\_depth} for the first model as a function of the number of cases. Examination of the chart shows that the value for the coefficient is still changing substantially at the end of 1000 cases. When the number of cases were increased to 2500 the results still did not seem very stable. When the cases increased again to 5000 there was a noticeable trend over the last thousand or so cases. When the cases were increased to 10~000, the estimated value over the last couple of thousand cases seems to have settled down, as can be seen in Fig \ref{Ex_2-convergence_of_mean}. So these results are based on 10~000 cases.

\begin{figure}[h!]
\centering
\includegraphics[width=4.5in]{FIGURES/ex2_cvg_plot-1.pdf}
\caption{Convergence of the mean}
...
\caption[Variables for which a 1~\% change causes a change of greater than 0.2~\% ]{Variables for which a 1~\% change causes a change of greater than 0.2~\% }
...
\noindent
\begin{longtable}{@{\extracolsep{\fill}}|l|l|l|l|l|l|}
\caption[Example 2 Table 2]{Example 2 Table 2}
...
\caption{Convergence of the mean for Example 3}
...
\caption{Standard error of Example 3}
\label{Ex_3-standard_error}
\end{figure}

The prelinary results, as seen in Fig \ref{Ex_3-convergence_of_mean} and \ref{Ex_3-standard_error}, suggest that we have enough cases. The averages all appear to have converged, and the standard errors are small.
...
\caption[Example 3 Table 1]{Example 3 Table 1}
...
The Table \ref{tbl:Ex_3-1} displays the number of cases where each of the alarms activated. Cases marked NA, are cases where the model itself did not converge. That was a very small number of cases for this example.
...
% This is the text that I have moved around. I don't understand what tex is doing with the layout.
To better understand the cases where the bedroom alarm fails to sound, a classification tree was generated, Fig \ref{Ex_3-classification_tree}, on whether the bedroom alarm sounded. It is unusual for a tree to produce results as stark as this one. What becomes clear on looking at the tree is that the state of the intervening doors determines whether the bedroom alarm sounds. If the fire is isolated from the bedroom by closed doors then the alarm will not sound. Otherwise it will.
...
\caption{Kernal Density estimate of the distribution}
...
\noindent
\begin{longtable}{@{\extracolsep{\fill}}|l|l|l|}
\caption[Example 3 Table 2]{Example 3 Table 2}
