...
\section{Output Control and Post-Run Calculations}
\label{info:OUTP}

The \ct{OUTP} namelist controls spreadsheet selection and validation formatting and can request summary calculations after a run. An \ct{OUTP} record containing only output-control parameters applies those options without creating a summary calculation.
\begin{description}
\item[\texttt{CRITERION}] Positive threshold used by \ct{TRIGGER_GREATER} or \ct{TRIGGER_LESSER}.
\item[\texttt{FILE}] Spreadsheet family containing the requested fields, such as \ct{NORMAL}, \ct{FLOW}, \ct{MASS}, \ct{SPECIES}, or \ct{WALL}.
\item[\texttt{FIRST\_DEVICE}] Legacy name for the first element of \ct{FIRST_FIELD}; retained for compatibility.
\item[\texttt{FIRST\_MEASUREMENT}] Legacy name for the second element of \ct{FIRST_FIELD}; retained for compatibility.
\item[\texttt{FIRST\_FIELD}] Required two-string spreadsheet column identifier. The first string identifies the object or category and the second identifies the measurement. For \ct{INTEGRATE}, it identifies simulation time.
\item[\texttt{FYI}] Optional descriptive text retained with the calculation but not used in the calculation.
\item[\texttt{ID}] Required unique identifier for a post-run calculation.
\item[\texttt{NET\_HEAT\_FLUX\_OUTPUT}] If \ct{T}, includes net target heat flux in spreadsheet output.
\item[\texttt{SECOND\_DEVICE}] Legacy name for the first element of \ct{SECOND_FIELD}; retained for compatibility.
\item[\texttt{SECOND\_MEASUREMENT}] Legacy name for the second element of \ct{SECOND_FIELD}; retained for compatibility.
\item[\texttt{SECOND\_FIELD}] Comparison or integration field. It is required for trigger and integration calculations.
\item[\texttt{SPREADSHEET\_OUTPUT}] String of output-category letters that selects and orders spreadsheet outputs. \ct{ALL} selects every category; \ct{NONE} or \ct{OFF} selects none.
\item[\texttt{TYPE}] Summary operation. Valid choices are \ct{TRIGGER_GREATER}, \ct{TRIGGER_LESSER}, \ct{MINIMUM}, \ct{MAXIMUM}, \ct{INTEGRATE}, and \ct{CHECK_TOTAL_HRR}.
\item[\texttt{VALIDATION\_OUTPUT}] If \ct{T}, writes abbreviated spreadsheet headings intended for automated validation processing.
\end{description}

The legacy \ct{DUMP} namelist accepts the post-run calculation parameters above but not the three output-control parameters. New files should use \ct{OUTP}.

The output of CFAST includes the temperatures of the upper and lower gas layers within each compartment, the ceiling/wall/floor temperatures within each compartment, the visible smoke and gas species concentrations within each layer, target temperatures and sprinkler activation times.  The amount of information can be very large, especially for complex geometries and long simulations.
...
\end{lstlisting}
The first column contains the compartment ID.  On each row, after its compartment number, the values listed from left to right are the upper layer temperature, lower layer temperature, the height of the interface between the two layers, the total pyrolysis rate, and finally the total fire size.  The only value given for the outside is the total heat release rate of fires venting to the outside.

\section{Detailed Outputs}

The following sections describe each of the outputs from the model.  Each section refers to a specific part of the printout and appears in the order the output appears. A description of each option follows. When running CFAST from within CEdit, an output file is automatically generated based on the input file name. For example, running the example file  \ct{Users_Guide_Example.in} generates an output file named  \ct{Users_Guide_Example.out}. All the detailed outputs described in the following sections are included in the output file.

\subsection{Output for Initialization}

This prints the initial conditions to the output before the actual run starts.  This merely mimics the inputs specified by the user in the input data file. The initial conditions break down into seven sections.  Each is described below with the section name. The following explanation uses the output from the case \ct{Users_Guide_Example.in}, which is included in the distribution.

\subsubsection{Overview}

The overview gives a general description of the case.  The output is fairly self-explanatory. ``Doors, ...'' is the total number of horizontal natural flow vent connections or wall vents in all compartments of the simulation.  ``Ceil. Vents, ...'' gives the total number of vertical natural flow vent connections or ceiling/floor vents in all compartments of the simulation.  The last header on the line ``MV Connections'' has the total number of mechanical flow connections to all compartments in the simulation. Times in these outputs are the times discussed in Sec.~\ref{info:TIME}. All times are in s.
...
\end{lstlisting}
All the information in this table comes from the compartments tab discussed in Sec.~\ref{info:COMP}.
...
\end{lstlisting}
From compartment, to compartment, vent number, width, sill height, and soffit height all come directly from the wall vent tab discussed in Sec.~\ref{info:VENT}. Vent opening and closing information can be specified by time, by an associated time/fraction history, or by an associated target that triggers a change in the vent opening based on temperature or heat flux; all are set in the same tab.
...
\end{lstlisting}
Top compartment, bottom compartment, shape, and area come from the Ceiling/Floor Vent tab discussed in Sec.~\ref{info:VENT2}. Relative height is the height of the vent above the floor of the bottom compartment and absolute height is the height of the vent above the base elevation.  Vent opening and closing information can be specified by time, by an associated time/fraction history, or by an associated target that triggers a change in the vent opening based on temperature or heat flux; all are set in the same tab.

\subsubsection{Mechanical Flow Connections}

This section lists all connections to compartments and fans that connect between compartments. The table lists, in order, the compartments connected by the fan and a numeric index assigned to the fan beginning with 1.  A fan actually draws air from the first or ``from'' compartment and pushes it to the second or ``to'' compartment. The fourth column is the cross-sectional area of the duct connection to the chosen compartment. Vent opening and closing information can be specified by time, by an associated time/fraction history, or by an associated target that triggers a change in the vent opening based on temperature or heat flux.
...
\end{lstlisting}
From compartment, to compartment, fan number, area, and flowrate are all set in the Mechanical Ventilation tab discussed in Secs.~\ref{info:VENT3} and \ref{info:VENT4}.  Vent opening and closing information can be specified by time, by an associated time/fraction history, or by an associated target that triggers a change in the vent opening based on temperature or heat flux; all are set in the same tab.
...
\end{lstlisting}
All of the inputs for all fires come from the fire specifications set in the Fires tab discussed in Sec.~\ref{info:FIRE}.  Units for most values are included in the output.  Fire position is in m, relative humidity is in \%, lower oxygen limit is in volume \%, and pyrolysis temperature is in K.
...
The entry for each detector or sprinkler shows the compartment and position of the device and its activation characteristics. For smoke detectors, activation is based on the smoke obscuration at the position of the detector; for heat detectors and sprinklers, the temperature of the detector.
...
All of the inputs for detectors and sprinklers come from the Detector / Suppression tab discussed in Sec.~\ref{info:DEVC2}. Units for position are all in m.
...
The reported species concentrations are percentages by volume. Optical depth per meter is a measure of the visibility in the smoke. Trace species (TS) is the total mass of the trace species that is present in the layer in kilograms. It is an absolute measure and not percent or density.
...
CFAST generates a number of output files in a plain text spreadsheet format.  These files capture a snapshot of the modeling data at an instant of time. This interval is set on the Simulation tab discussed in Sec.~\ref{info:TIME}. \emph{However}, there are events which can occur in between these reporting periods. Examples are the ignition of objects and the activation of detectors or sprinklers. These are \emph{not} reported in these output files.
...
\item[Opening Fraction]: fraction the vent is open. Zero indicates a fully-closed vent; one indicates a fully-opened vent.
...
\item[68, mvent] Exceeded one of the array bounds, ierror=68 (external), 69 (internal)  and 70 (fan)
...
\item[74, setp] The requested “SETP” command does not exist
