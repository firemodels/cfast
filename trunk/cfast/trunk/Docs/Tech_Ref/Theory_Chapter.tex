...
Since $f_\C$ is just the mass ratio of hydrogen to carbon in the fuel, we can solve for $\dm_\COTWO$ as
...
Since $y_\CO$/$y_\COTWO$, $y_\So$/$y_\COTWO$, $H$/$C$, $O$/$C$, $y_\HCl$, and $y_\HCN$ are direct CAST inputs, production rates for these follow from eq \ref{eqCFASTcarbonbalance}. $y_\CO$/$y_\COTWO$, $y_\So$/$y_\COTWO$, $y_\HCl$, and $y_\HCN$ are typically available from material test data. $H$/$C$ and $O$/$C$ are properties of the fuel.   
