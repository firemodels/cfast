...
\subsection{Gas Tenability (\texorpdfstring{\textct{gas\_tenability}}{gas\_tenability})}

For the same scenario as the previous example, gas tenability can be calculated.  A steady-state concentration of 0.0223~\% by volume of CO$_2$, and 0.00137~\% by volume of CO is reached. A 1 s increment of asphyxiant gas FED can by calculated by
...
The following figure compares the calculated FED to its expected value.

\begin{figure}[ht]
\centering
\includegraphics[width=3.0in]{SCRIPT_FIGURES/Verification/gas_tenability}
\caption[Results of the test case {\ct methane\_flame\_simple.in}]{Expected and CFAST calculated values for fractional effective dose in a one room test case {\ct gas\_tenability.in}.}
