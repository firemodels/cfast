...
Air is supplied to a single compartment at ambient conditions via a single mechanical vent with a flowrate of 0.01 m$^3$/s. Wall or floor leakage is specified in two test cases. Flow out the leak should equal the specified flow in through the mechanical vent.  The expected mass flow rate is just the specified volumetric flow times the density of air at ambient conditions for an expecte value of 0.012 kg/s. The expected results and CFAST results for the wall leak and floor leaks are shown in Figures~\ref{Leak_Result1} and \ref{Leak_Result_2}, respectively.
