...
$\boldsymbol{y} =$& $(P_1, P_2, ..., P_{n-1}, P_n,$ & (pressures for each compartment)  \\
               ...
               &  $P_{MV_1}, P_{MV_2}, ..., P_{MV_{m-1}}, P_{MV_m}$, & (pressures at each interior node of the mechanical ventilation system)  \\
               &  $T_{MV_1}, T_{MV_2}, ..., T_{MV_{m-1}}, T_{MV_m} )$ & (temperatures at each segment of the mechanical ventilation system) \\
\end{tabular}
\end{center}

In the simplest case, all temperatures are initialized to ambient, all pressures are initialized to zero, and $\boldsymbol{y'}$ is zero. For more complex cases, an initial $y$ and $y'$ at $t=0$ is calculated and supplied to DASSL. At each time step, the solver estimates values for $y$ and $y'$ at $t+\Delta t$, CFAST calculates an actual $y'$ from the estimated values of $y$ and returns the difference between the estimated $y'$ and calculated $y'$ to DASSL. DASSL uses backward differentiation formulas of orders one through five to solve for $y$ and $y'$ to within tolerances specified by CFAST \cite{DASSL:1982,DASSL:1989}.
