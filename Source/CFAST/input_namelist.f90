...
5002  format ('***Error: Not a compatible version, input file written in CFAST', i3, ' running on CFAST' i3)
...
34          if (ios>0) then
               write(iofill, '(a,i3)') '***Error in &DEVC: Invalid specification for inputs. Check &DEVC input, ' , n_targets+n_detectors
               stop
            end if
      end do devc_loop
...
34       if (ios>0) then
      ...
      real(eb) :: area,co_yield,heat_of_combustion,height,pf_co_yield,pf_soot_yield,pf_trace_yield,radiative_fraction,setpoint,soot_yield,trace_yield
...
34       if (ios>0) then
      ...
      read_fire_loop: do ii=1,n_fires
        ...
        if (trim(ignition_criterion) /= 'NULL') then
         if (trim(ignition_criterion)=='TIME' .or. trim(ignition_criterion)=='TEMPERATURE' .or. trim(ignition_criterion)=='FLUX') then
...
5000  format ('***Error: the key word ',a5,' is not part of a fire definition. fire keyword are likely out of order')
      ...
      vent_loop: do
         ...
         34 if (ios>0) then
               write(iofill, '(a,i3)') '***Error in &VENT: Invalid specification for inputs. Check &VENT input, ', n_hvents+n_mvents+n_vvents
               stop
            end if
      end do vent_loop
