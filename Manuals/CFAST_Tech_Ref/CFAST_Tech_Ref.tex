...
\addbibresource{../Bibliography/CFAST_refs.bib}
...
CFAST was originally developed by Walter Jones, Richard Peacock, Glenn Forney, and Paul Reneke.
...
\item[Chapter 5] documents the calculation of heat transfer including radiation exchange between fires, walls, gas layers, and objects within compartments, convection between gases and compartment bounding surfaces or objects within compartments, and conduction in compartment bounding surfaces and objects within compartments.
...
\be
   \dq'' +k \, \frac{\partial \Ts(0,t)}{\partial x}=0   \quad ; \quad  \Tw(t) \equiv \Ts(0,t) \label{bc}
\ee
where $\partial \Ts(0,t)/\partial x$ is the temperature gradient at the wall surface, $\dq''$ is the net radiative and convective heat flux from the adjacent gas layer (see Chapter~\ref{chap:heat_transfer} for a description of the heat flux term) and for brevity the temperature wall profiles are referred to collectively as $\Ts(x,t)$. The spatial coordinate $x$ refers to the depth inside the wall; $x=0$ is the wall surface. The temperature gradient in Eq.~(\ref{bc}) is determined by first updating the temperature profile by solving the heat conduction equation
\begin{equation}
   \frac{\partial \Ts}{\partial t} = \frac{k}{c \, \rho} \frac{\partial^2 \Ts}{\partial x^2}
\end{equation}
where $\rho$, $c$ and $k$ are the density, specific heat and thermal conductivity of the wall material. Further details of the solution procedure are included in Appendix~\ref{chap:heatsolution}.
...
\begin{eqnarray}
  ...
  &+& \; \nu_\So \, \mathrm{Soot} \; + \; \nu_\HCl \mathrm{HCl} \; + \; \nu_\HCN \mathrm{HCN} + \nu_\NTWO \mathrm{N}_2 \label{stoich}
\end{eqnarray}
...
\begin{eqnarray}
  \nu_\So &=& \frac{M_\F}{M_\So} \; y_\So \label{soot_yield} \label{nu_s} \\[.1in]
...
Using the specified heat release rate of the fire, $\dQ$, and a user-specified heat of combustion, $\Dh$,  the model calculates the pyrolysis rate of fuel, $\dmf$:
\be
   \dmf = \frac{\dQ}{\Dh}
\ee
In the event that the HRR is constrained by the availability of oxygen, it is assumed that the pyrolysis rate does not change, but that only part of the pyrolyzed fuel burns and the HRR becomes:
\be
...
The potential combustion products, CO$_2$, CO, H$_2$O, soot, HCN, and HCl, of the unburned hydrocarbons (UHC) are transported to the upper layer via entrainment. This is done for numerical convenience to limit the amount of storage required to track the potentially large number of fuel gases specified by the user. Should sufficient oxygen become available in the upper layer, or some of the upper layer gases spill over into another compartment, the stoichiometric coefficients of a surrogate fuel reaction are calculated, as here for CO$_2$:
\begin{equation}
   \nu_\COTWO = \frac{m_\COTWO}{m_\UHC} \frac{M_\UHC}{M_\COTWO}
\end{equation}
The molecular formula of the surrogate fuel is found by solving Eqs.~(\ref{nu_s}) through (\ref{nu_N2}) inversely to obtain the atom counts of the fuel molecule. Combustion is computed using the same procedure that is used for the original fires. The heat of combustion of the surrogate fuel reaction is the mass-weighted average of the unburned hydrocarbons.
...
\ee
To account for the presence of an upper layer, the background temperature, $T_\infty$, in Eq.~(\ref{Tcj}) is replaced with the upper layer temperature, $\Tu$. No adjustment needs to be made to Eq.~(\ref{QsH}) because $\rho_\infty \, T_\infty$ is assumed constant.
...
The previous paragraph explains from where the gas flowing through the vent is extracted. Where the gas goes is simpler. If the temperature of the hot gas layer of the lower compartment is greater than the temperature of the lower layer of the upper compartment, then the mass and heat are deposited into the upper layer of the upper compartment. Otherwise, the mass and heat are deposited into the lower layer of the upper compartment.
...
The term $F_{j-i}$ is the configuration factor (fraction of radiant energy emitted by surface $j$ that is intercepted by surface $i$), $\tau_{j-i}$ is the transmittance, $\sigma$ is the Stefan--Boltzmann constant, $\epsilon_i$ is the emissivity, $\Ti$ is the temperature, and $c_i''$ is the radiative flux from gas layers and fire to surface $i$.
...
\be
   F_{i-j} = \frac{1}{A_i} \int_{A_i} \int_{A_j} \frac{\cos \theta_i \, \cos \theta_j}{\pi L^2} \, \d A_j \, \d A_i \label{eq:config_factor}
\ee
...
To compute a solid angle, consider the triangle defined by the vertices $\mathbf{v}_1$, $\mathbf{v}_2$, and $\mathbf{v}_3$ as illustrated in Fig.~\ref{solidanglesetup}. The solid angle of this triangle with respect to the origin is computed as
\begin{eqnarray*}
...
\be
   \dqc'' = h \, \brackets{\Tg - \Tw}  \label{convective_heat_flux}
\ee
...
\be
   h = C {|\Tg - \Tw|}^{1/3}
\ee
where $C$ is an empirical coefficient (1.52 for the floor and ceiling (in the absence of a ceiling jet) and 1.31 for the walls~\cite{Holman:1990}), $\Tg$ is the average gas layer temperature adjacent to the surface, and $\Tw$ is the surface, or \underline{w}all, temperature.
...
\be
   \dq_{\rm c,avg}'' = \frac{1}{LW} \int_0^{2\pi} \int_0^R \dqc'' \, r \, \d r \, \d\theta = \frac{0.27 \, \dQ_{\rm c}}{(LW)^{0.68} \, H^{0.64}} \label{eq:cjfluxavg}
\ee
...
\be
   h = \max \left( \frac{\dq_{\rm c,avg}''}{\Tu-\Ts} \, , \, C {|\Tu - \Tw|}^{1/3} \right)
\ee
...
The heat conduction equation is solved in the direction normal to wall and target surfaces using non-uniformly spaced nodes and a second-order accurate central difference scheme for the spatial derivatives.

Wall surface temperatures are updated in time implicitly using the solver DASSL, which couples wall surface temperatures with gas solution variables (pressures, layer volumes and temperatures). Target temperatures, on the other hand, are updated in time using a semi-implicit time marching scheme. Because targets do not affect compartment conditions significantly, there is no need to couple them with the gas phase solution variables as is done with wall temperatures.

At each time step, the wall temperature profiles are updated in time until the net convective and radiative heat flux striking the wall equals the heat flux into the solid~\cite{Moss:1992}:
\be
   \dq'' \equiv \dqr'' + \dqc'' = -k \, \frac{\partial \Ts}{\partial x} \Big|_{x=0} \label{eq:net_fluxes}
\ee
where $k$ is the thermal conductivity of the wall material.  This solution strategy requires a differential algebraic equation (DAE) solver that can simultaneously solve both differential and algebraic equations.  With this method, only one or two extra equations are required per wall segment (two if both the interior and exterior wall segment surface temperatures are computed).  This solution strategy is more efficient than the method of lines because fewer equations need to be solved.

Wall and target inner temperature profiles are advanced in time by solving the heat conduction equation normal to the solid surface:
\be \frac{\partial \Ts}{\partial t} = \frac{k}{\rho c}\frac{\partial^2 \Ts}{\partial x^2}
...
\be
   \Ts(0,t) \equiv \Tw(t)
\ee
Walls are sub-divided into 30 cells; 20 of which span the inner half-thickness and 10 which span the outer half-thickness. The cell width increases linearly starting from the front and back surfaces. Targets are sub-divided into 60 cells; half of which span each half-thickness and increase by 50~\% starting from the front or back surfaces.
...
The 1-D heat conduction equation can be written in either Cartesian or cylindrical\footnote{Only targets can take on a cylindrical geometry, and the solution of the heat conduction equation in cylindrical coordinates uses a uniform grid.} coordinates. The solution methodology shall be presented for cylindrical coordinates:
\be
  \frac{\partial \Ts}{\partial t} = \frac{k}{\rho c} \frac{1}{r} \frac{\partial}{\partial r} \left( r \frac{\partial \Ts}{\partial r} \right)
\ee
Dividing the cylinder into $N$ concentric control volumes, this equation can be written in discretized form, where $T_i^n \equiv \Ts(x_i,t_n)$:
\begin{eqnarray}
...
\be
   \frac{\d \TL}{\d t} = \frac{\sqrt{v}}{\rm RTI} \brackets{\Tg - \TL}  \label{eq:RTI}
\ee
...
\item CFAST can also model a smoke detector as a very sensitive heat detector with an activation temperature rise of 5~$^\circ$C and RTI of 5~(m$\cdot$s)$^{1/2}$. The temperature rise estimate is based on a study performed by Bukowski and Averill~\cite{Bukowski:1998}. The RTI is not based on experiment; it simply provides a slight time delay in activation.
\end{itemize}
Users are cautioned that both of these models are very crude and the uncertainty in predictions is substantial. Consult the CFAST Validation Guide for more details on their validation.
...
\ee
where $\dq''$ is the radiant heat flux in kW/m$^2$ and $T$ is the temperature in \degc. For target locations where the radiant flux is less than 2.5 kW/m$^2$, the radiant heat term is taken to be zero consistent with ISO 13571.
...
$\Tl$               & \underline{l}ower layer gas temperature, K \\
$\Tp$               & \underline{p}lume temperature, K \\
$\Ts$               & \underline{s}olid internal temperature, K \\
$\Tu$               & \underline{u}pper layer gas temperature, K \\
$\Tw$               & \underline{w}all (ceiling, upper wall, lower wall, floor) surface temperature, K \\
$t$                 & time, s \\
...
To derive a differential equation for compartment pressure, rewrite Eq. (\ref{eq:afirst_law}) for the upper and lower layers to obtain
\begin{eqnarray}
...
CFAST uses the differential~/~algebraic solver DASSL~\cite{DASSL:1982,DASSL:1989} to solve the zone fire modeling equations Eqs.~(\ref{eq1}) through Eq.~(\ref{bc}).
...
The equations are written in the form of a root finding problem
\begin{equation}
\vecF\brackets{t,\vecy,\vecy'}=0 \quad ; \quad \vecy' = \frac{ d\vecy}{dt}
\end{equation}
where $\vecF$ is given by
\begin{equation}
\vecF(t,\vecy,\vecy')=\left\{
\begin{array}{ll}
  \vecy'-\mathbf{f}(t,\vecy) & \hbox{gas phase} \\[.1in]
  \dq''(\vecy) +k \, \frac{\partial T_{\rm s}(0,t)}{\partial x} & \hbox{gas/solid interface} \label{eq:resid}
\end{array}
\right.
\end{equation}
$\mathbf{f}$ is the right hand side of Eqs. (\ref{eq1}) through (\ref{eq4}), $t$ is the time, $\vecy$ is the solution vector consisting of the compartment pressures, $P_i$, upper layer temperatures, $T_{{\rm u},i}$, upper layer volumes, $V_{{\rm u},i}$, lower layer temperatures, $T_{{\rm l},i}$, and the ceiling, upper wall, lower wall, and floor surface temperatures, $T_{{\rm w},i}$:
\begin{equation}
   \vecy = (P_1, ... , P_n, T_{{\rm u},1}, ... , T_{{\rm u},n} , V_{{\rm u},1}, ... , V_{{\rm u},n}, T_{{\rm l},1}, ... ,T_{{\rm l},n}, T_{{\rm w},1}, ..., T_{{\rm w},4n} )
\end{equation}
The wall surface temperatures, $T_{{\rm w},i}$, are components of the solution vector, $\vecy$, but also serve as boundary conditions for the solid phase temperature equation
\begin{equation}
   \frac{\partial T_{\rm s}}{\partial t} = \frac{k}{c \, \rho} \frac{\partial^2 T_{\rm s}}{\partial x^2} \quad ; \quad T_{\rm s}(0,t) \equiv T_{\rm w}(t) \label{eq:fouriera}
\end{equation}
The coupling of gas and solid phase requires that the convective and radiative heat flux, $\dq''(\vecy)$, equal the temperature gradient at the wall surface, $\partial T_{\rm s} / \partial x$, where $x$ is the one-dimensional spatial coordinate pointing into the solid. The expression for the heat flux is derived in Chapter~\ref{chap:heat_transfer}.

An outline of the solution procedure within DASSL is as follows:
\begin{enumerate}
\item Assuming that $\vecy_n$ and $\vecy'_n$ are known at time $t=t_n$, advance the solution for a time step, $\Delta t$, using a backward differential formula chosen by the DASSL solver.
\label{step:first}
\item Using the estimated solution, $\vecy_{n+1}$, compute the right hand sides of Eqs.~(\ref{eq1}) through (\ref{eq4}), $\mathbf{f}(t_{n+1},\vecy_{n+1})$.
\label{step:rhs}
\item Compute the gas phase residuals, $\vecy'_{n+1}-\mathbf{f}(t_{n+1},\vecy_{n+1})$, using the finite difference approximation of $\vecy'_{n+1}$ computed in step~\ref{step:first} and the right hand side of the gas phase equations computed in step~\ref{step:rhs}.
\label{step:residode}
\item Using the updated wall surface temperatures, $T_{{\rm w},i}$, estimated in step~\ref{step:first} as a boundary condition, advance the temperature profiles, $T_{{\rm s},i}(x,t)$, from $t=t_n$ to $t=t_{n+1}$ using an implicit time update of Eq.~(\ref{eq:fouriera}).
\item Compute the residuals $q_i''+k\dbydx{T_{{\rm s},i}}$ using the heat flows computed in step \ref{step:rhs} and the surface temperature gradients computed using Eq.~\ref{eq:fouriera}.
\item Check if all the residuals are within a specified error tolerance. If not, continue the root finding procedure until convergence is achieved.
\end{enumerate}
