...
A system of ordinary differential equations for pressure, upper layer volume, upper and layer temperature are derived in Appendix \ref{chap:derivation}
 and given below:
...
For convenience, Eq. \ref{eq:first_law} used in the following derivations is re-written here:
\be
   \ddt\left(c_v m_i T_i\right)   =  \qdot_i - P \dbydt{V_i}\label{eq:afirst_law}
\ee

To derive a differential equation for compartment pressure, rewrite Eq. \ref{eq:afirst_law} for the upper and lower layer and substitute $\rho_i=m_i/V_i$ to give
\begin{eqnarray}
