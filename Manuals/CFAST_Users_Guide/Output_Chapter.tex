...
\item[\texttt{NET\_HEAT\_FLUX\_OUTPUT}] If \ct{.TRUE.}, includes net target heat flux in spreadsheet output.
...
\item[\texttt{VALIDATION\_OUTPUT}] If \ct{.TRUE.}, writes abbreviated spreadsheet headings intended for automated validation processing.
\end{description}
...
\end{lstlisting}
All the information in this table comes from the compartments tab discussed in section  \ref{info:COMP}.
...
\end{lstlisting}
From compartment, to compartment, vent number, width, sill height, and soffit height all come directly from the wall vent tab discussed in section \ref{info:VENT}. Vent opening and closing information can be specified by time, by an associated time/fraction history, or by an associated target that triggers a change in the vent opening based on temperature or heat flux; all are set in the same tab.
...
\end{lstlisting}
From compartment, to compartment, fan number, area, and flowrate are all set in the Mechanical Ventilation tab discussed in sections \ref{info:VENT3} and \ref{info:VENT4}.  Vent opening and closing information can be specified by time, by an associated time/fraction history, or by an associated target that triggers a change in the vent opening based on temperature or heat flux; all are set in the same tab.
...
Material choices for the ceiling, walls, and floors are discussed in section \ref{info:COMP2}. Setting thermophysical properties is done in the Thermal Properties tab discussed in section \ref{info:MATL}. Units for thermal properties are standard S.I. units.  For thermal conductivity,  kW/(m$\cdot$K); for specific heat,  kJ/(kg$\cdot$K); for density, kg/m$^3$; for thickness, m; emissivity is dimensionless.
...
All of the inputs for detectors and sprinklers come from the Detector / Suppression tab discussed in section \ref{info:DEVC2}. Units for position are all in m.
