...
This document provides documentation for creating and running Monte Carlo simulations using the Consolidated Fire And Smoke Transport (CFAST) model, following the general framework set forth in the ``Standard Guide for Evaluating the Predictive Capability of Deterministic Fire Models,'' ASTM~E~1355~\cite{CFAST:ASTM:E1355}. Instructions for using CFAST are contained in a separate user's guide, and model assessment information is contained in a separate verification and validation guide.
...
This publication introduces the CFAST Fire Data Generator or CData that has been developed by NIST as a tool to help users do Monte Carlo analysis. The various parts of CData are described and then a very simple analysis is first presented to demonstrate the different parts of the analysis that CData can assist in preforming. This is followed by three slightly more complex analsyses to further demonstrate the uses of CData.

Included in CData is the statisical software R. It is included so that CData can provide a number of built in tools to analize the data. Two of the tools are useful in determining that enough cases have been run to draw justifable conclusions from the analysis. The first, convergence of the mean,
...
The scripts listed below are in R\footnote{R Core Team (2020). R: A language and environment for statistical computing. R Foundation for Statistical Computing, Vienna, Austria. URL \url{https://www.R-project.org/}.} and are in the form of R Markdown files.\footnote{Yihui Xie and J.J. Allaire and Garrett Grolemund (2018). R Markdown: The Definitive Guide. Chapman and Hall/CRC. ISBN 9781138359338. URL \url{https://bookdown.org/yihui/rmarkdown}}
