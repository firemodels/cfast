...
\caption{Sample CFAST visualization of a single compartment structure use for example 1.}
...
\begin{description}
\item[Compartment dimensions:] uniform distribution scaling all compartment dimensions from 2 m x 2 m x 2 m to 10 m x 10 m x 10 m.
\item[Vent dimensions:] uniform distribution scaling vent bottom from 0 m to 1.5 m; uniform distribution scaling vent top from 2 m to 9 m limited by compartment height; uniform distribution scaling vent width from 0.25 m to 9 m limited by compartment width.
\item[Fire growth rate:] uniform distribution ranging from 75 s to 1 MW to 600 s to 1 MW. Peak heat release rate set to 10 MW to ensure flashover in the compartment when there is sufficient oxygen.
\end{description}
...
\begin{description}
\item[Fire location:] random placement in the center of any of the compartments in the structure.
\item[Initial fire growth rate:] fire grows from zero to a normal distribution of 23 kW $\pm$ 7 kW in 207 s $\pm$ 46 s.
\end{description}

\subsection{Example 3: Model Sensitivity}

For the final example we look at the sensitivity of changing model inputs on important output variables in an office fire scenario.
