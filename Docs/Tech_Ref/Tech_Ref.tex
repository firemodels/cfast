...
\be
   \ddt\left(c_v m_i T_i\right) +  P \dbydt{V_i} =  \dot h_i \label{eq:first_law}
\ee

The enthalpy source term, $\dot h_i$, consists of the fire's heat release rate, conduction losses to walls, and radiation exchange. The layer temperature and density are related to the compartment pressure via the ideal gas law:
\be
...
\begin{eqnarray}
   c_v\ddt\left( V_l\rho_l T_l\right) +  P \dbydt{V_l} &=  &\dot h_l \label{eq:lower}\\
   c_v\ddt\left( V_u\rho_u T_u\right) +  P \dbydt{V_u} &=  &\dot h_u \label{eq:upper}
\end{eqnarray}
...
\begin{eqnarray}
   \frac{c_vV}{R}\dbydt{ P}  &=  &\dot h_l + \dot h_u
\end{eqnarray}

\noindent Finally use $R=c_p-c_v$ and $\gamma=c_p/c_v$ to show that $R/c_v=\gamma-1$ giving

\begin{eqnarray}
   \dbydt{ P}  &=  &\frac{\gamma-1}{V}\left(\dot h_l + \dot h_u\right)
   \label{eq:ODE_P}
\end{eqnarray}
...
\label{eq:vua}
\end{equation}
\noindent Subsituting Eq. \ref{eq:vua} into the upper layer form Eq. \ref{eq:first_law} ($i=u$) gives
\begin{eqnarray}
\dot h_u&=&\frac{1}{\gamma-1}\ddt\left(V_uP\right) + P\dbydt{V_u}\\
...
\begin{equation}
\dbydt{V_u} = \frac{1}{P\gamma}\left((\gamma-1)\dot h - V_u\dbydt{P}\right)
\label{eq:ODE_VU}
\end{equation}
...
\begin{equation}
P\dbydt{Vu} = \frac{\gamma-1}{\gamma} \dot h - \frac{V_u}{\gamma}\dbydt{P}
\label{eq:pvu}
\end{equation}
...
\begin{eqnarray}
   \ddt\left(c_v m_u T_u\right) +  P \dbydt{V_u} &=  &\dot h_u\\
c_v\dot m_uT_u+c_vm_u\dbydt{T_u}+\frac{\gamma-1}{\gamma} \dot h - \frac{V_u}{\gamma}\dbydt{P}&=&\dot h_u
\label{eq:tua}
\end{eqnarray}
Multiplying Eq. \ref{eq:tua} by $\gamma$ gives
\begin{equation}
c_p\dot m_uT_u+c_pm_u\dbydt{T_u} - \frac{V_u}{\gamma}\dbydt{P}=\dot h_u
\end{equation}
Finally, solving for $\dbydt{T_u}$ gives
\begin{equation}
\dbydt{T_u}=\frac{1}{c_pm_u}\left( \dot h_u-c_p\dot m_uT_u+ \frac{V_u}{\gamma}\dbydt{P}\right)
\label{eq:ODE_TU}
\end{equation}
A differential equation for lower layer temperature
\begin{equation}
\dbydt{T_l}=\frac{1}{c_pm_l}\left( \dot h_u-c_p\dot m_lT_l+ \frac{V_l}{\gamma}\dbydt{P}\right)
\label{eq:ODE_TL}
\end{equation}
...
The mass, $\dmau$ and $\dmal$, and enthalpy, $\dhu$ and $\dhl$, terms are simply the net sum of all mass and enthalpy flowing into a layer (through vents, from fires, between layers via a plume, or by radiation and convection from compartment surfaces).
