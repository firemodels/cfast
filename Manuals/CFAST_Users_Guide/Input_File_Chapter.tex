...
To calculate heat loss through the ceiling, walls, and floor of a compartment, material properties must be specified. Separate properties can be specified for the ceiling and floor, but the four walls all must have the same set of properties. The thermophysical properties are described in Sec.~\ref{info:MATL}. If the thermophysical properties of the surfaces are not specified, they will be treated as adiabatic, i.e. no heat transfer. The back surfaces of compartments are assumed to be exposed to ambient conditions unless specifically specified (see the section on Surface Connections to specify heat transfer connections between compartments).
...
\item[Position X, Y] (default units: \unit{m}, default value: compartment center): Position of the center of the base of the fire relative to the front left corner of the compartment.
...
\item[\texttt{LOCATION}] Required X and Y coordinates in \unit{m} relative to the compartment origin. Negative coordinates are interpreted from the opposite compartment boundary when that compatibility option is enabled.
\item[\texttt{SETPOINT}] Ignition time in s, target temperature in \unit{\degreeCelsius}, or target incident heat flux in \unit{kW/m^2}. A value not greater than zero ignites the fire initially.
\end{description}
...
\begin{description}
\item[\texttt{AREA}] (units: \unit{m^2}, default value: 0~\unit{m^2}): Constant area of the fire base. CFAST treats values smaller than the area of a 0.2~m diameter circle as that minimum area. A \ct{TABL} column labeled \ct{AREA} overrides this value.
...
\item[Material] The material the target is made of or OFF for no heat conduction calculation. Any material defined on the Thermal Properties tab (Section~\ref{info:MATL}) may be used here. There can be only one material per target.
...
\item[RTI] (default units: \unit{(m.s)\tothe{1/2}}, default value: none): the Response Time Index (RTI) for the sprinkler or detection device.

\item[Spray Density] (default units: \unit{m/s}, default value: none): the amount of water dispersed by a sprinkler.  The units for spray density are length/time, derived by dividing the volumetric flow rate by the spray area. The suppression calculation is based upon an experimental correlation by Evans~\cite{Evans:1993}.
\end{description}
...
\section{\ct{DEVC} Inputs}

Targets, sprinklers, and detector inputs are written to the \ct{DEVC} namelist.

\begin{description}
    \item[\texttt{ADIABATIC\_TARGET}] If \ct{T}, applies an adiabatic-surface-temperature target calculation using the emissivity of the assigned \ct{MATL_ID}. Supply \ct{CONVECTION_COEFFICIENTS} for this calculation.
    \item[\texttt{COMP\_ID}] Required identifier of the compartment containing the device.
    \item[\texttt{CONVECTION\_COEFFICIENTS}] Front- and back-face convection coefficients in \unit{kW/(m^2 K)}.
    \item[\texttt{DEPTH\_UNITS}] Character string defining how \ct{TEMPERATURE_DEPTH} is interpreted. If \ct{FRACTION}, \ct{TEMPERATURE_DEPTH} is a fraction of target thickness. If \ct{M}, \ct{TEMPERATURE_DEPTH} is the actual depth in \unit{m}.
    ...
    \item[\texttt{SETPOINT}] Activation temperature in \unit{\degreeCelsius} for a heat detector or sprinkler, or obscuration in \unit{\%/m} for a smoke detector. The source default is 57~\unit{\degreeCelsius} for both heat detectors and sprinklers and 23.93~\%/m for smoke detectors.
    ...
    \item[\texttt{SURFACE\_TEMPERATURE}] Initial target surface temperature in \unit{\degreeCelsius}; default is the initial interior ambient temperature.
...
For wall connections the inputs are:

\begin{description}
    \item[First Compartment] First of the compartments whose walls are connected.

    \item[Second Compartment] Second of the compartments whose walls are connected.
...
When specifying a horizontal connection, a connection must be defined for both directions. The specified area fractions should result in the same connected area. Additionally, for any single compartment the sum of area fractions for all the connected vertical surfaces cannot exceed unity. If the sum is less than unity, the unallocated fraction is assumed to be connected to the ambient. As an example consider two compartments that share a wall where the first compartment is 1~\unit{m} x 1~\unit{m} x 1~\unit{m}  and the second compartment is 2~\unit{m} x 2~\unit{m}  x 2~\unit{m} . The total wall area for the first compartment is 4~\unit{m^2}. The total wall area for the second compartment is 16~\unit{m^2}. If the first compartment shares one entire wall, then the fraction would be 0.25 (1~\unit{m^2}/4~\unit{m^2}).  For the second compartment the fraction would be 0.0625 (1~\unit{m^2}/16~\unit{m^2}).

For ceiling/floor connections the inputs are:

\begin{description}
    \item[Top Compartment] The top of the two compartments to be connected by a ceiling/floor heat transfer connection. The connection is through the floor of this compartment.

    \item[Bottom Compartment] The bottom of the two compartments to be connected by a ceiling/floor heat transfer connection. The connection is through the ceiling of this compartment.
\end{description}

Connection inputs are written to the \ct{CONN} namelist. The available parameters are:

\begin{description}
\item[\texttt{COMP\_ID}] The source compartment. For a ceiling/floor connection, this is the upper compartment.
\item[\texttt{COMP\_IDS}] An array of destination compartment identifiers. A wall connection may list multiple compartments or \ct{OUTSIDE} for the ambient. A ceiling/floor connection uses only the first value and it corresponds to the lower compartment.
\item[\texttt{F}] An array of wall-area fractions corresponding to \ct{COMP_IDS}; each value must be between 0 and 1. These fractions are based on the wall area for \ct{COMP\_ID}. This parameter is not used for ceiling or floor connections. Up to 200 connections can be defined for each compartment.
\item[\texttt{TYPE}] An array of connection types: \ct{WALL}, \ct{CEILING}, or \ct{FLOOR} corresponding to \ct{COMP_IDS}. The two vertical types are processed identically (i.e., \ct{COMP_ID} is always the upper compartment).
\end{description}
