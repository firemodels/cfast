...
\begin{description}
\item[Visualization Type] (default: 2-D) There are three choices for this parameter. \textbf{2-D} specifies a single plane slice of temperature at the specified \textbf{Position} and \textbf{Axis}. \textbf{3-D} specifies slices in all three directions and all positions. \textbf{ISOF} specifies a surface of constant gas temperature. See the Smokeview documentation~\cite{Smokeview_Users_Guide_6} for details on the use of Smokeview.
\item[Compartment] (default: \textbf{All}; namelist: \ct{COMP_ID}) Visualizations can be placed in a single compartment or at the same position and axis in all compartments.
\item[Position] (default: \textbf{Depth/2} m; namelist: \ct{POSITION}) For a 2-D slice, position along the specified axes where the slice is placed measured from the compartment origin for the selected axis (0,0,0 is the bottom left corner of the compartment). See page \pageref{Compartment_Geometry}.
\item[Axis] (default: \textbf{X-axis (Width)}; namelist: \ct{PLANE}) Axis perpendicular to the specified slice.
\item[Temperature] (default: 20~\unit{\degreeCelsius}; namelist: \ct{VALUE}) Specified isosurface temperature.
\end{description}
Examples of the three visualization types are given here:
\begin{lstlisting}
&SLCF DOMAIN = '2-D', COMP_ID = 'Comp 1', PLANE = 'X', POSITION = 1.8 /
&SLCF DOMAIN = '3-D', COMP_ID = 'Comp 1' /
&ISOF COMP_ID = 'Comp 1', VALUE = 20 /
\end{lstlisting}
...
By default, slice (contour) files are generated with a grid of 50 points in each direction for each compartment specified. The grid spacing can be adjusted individually by compartment. Specifying a larger number of data points can \textit{dramatically} slow program execution since the gas temperature and velocity are evaluated at each grid location every time a Smokeview output is specified.  The default value should be appropriate for most simulations.
\begin{description}
\item[Width (cells)] (default: 50; namelist: \ct{COMP/GRID(1)}) Contour slice points in the $x$ direction.
\item[Depth (cells)] (default: 50; namelist: \ct{COMP/GRID(2)}) Contour slice points in the $y$ direction.
\item[Height (cells)] (default: 50; namelist: \ct{COMP/GRID(3)}) Contour slice points in the $x$ direction.
\end{description}
Sample visualizations are included below.

\begin{figure}[h!]
\begin{center}
\includegraphics[width=3.5in]{FIGURES/SMV_Sample}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=3.5in]{FIGURES/SMV_Temperature}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=3.5in]{FIGURES/SMV_Velocity}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=3.5in]{FIGURES/SMV_Detectors}
...
\begin{description}
    \item[Spreadsheet Output] (default: \textbf{ALL}; namelist: \ct{SPREADSHEET\_OUTPUT}) String of output-category letters that selects and orders spreadsheet outputs. \ct{ALL} selects every category.
    \item[Net Heat Flux Output] (default: \ct{F}; namelist: \ct{NET_HEAT_FLUX_OUTPUT}) If \ct{T}, includes net target heat flux in spreadsheet output.
    \item[Validation Output] (default: \ct{F}; namelist: \ct{VALIDATION\_OUTPUT}) If checked, abbreviated headings for spreadsheet columns facilitate the post-processing of validation cases.
    \item[Debug Output] (default: \textbf{OFF}; namelist: \ct{DIAG/DEBUG_PPRINT}, \ct{DIAG/RESIDUAL_DEBUG_PRINT}) If checked, CFAST will create a detailed output spreadsheet that contains values of all the solution variables at each successful solution time step. This file is typically only of use to model developers diagnosing a problem with the model.
\end{description}




\chapter{Advanced Diagnostics}
\label{info:DIAG}


The following miscellaneous parameters are used for diagnostics and testing, not typical fire scenarios. They can only be entered manually in the input file using a single \ct{DIAG} namelist record.
\begin{description}
...
\item[\texttt{VERIFICATION\_TIME\_STEP}] Time step used by verification calculations.
\end{description}
