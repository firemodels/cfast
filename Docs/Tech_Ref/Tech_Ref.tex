...
\end{eqnarray}
These equations are supplemented by four partial differential equations per compartment governing the temperature in depth of the ceiling, upper wall, lower wall, and floor, which for the sake of brevity will be referred to collectively as $\Tw(x,t)$. The spatial coordinate $x$ refers to the depth inside the wall; $x=0$ is the wall surface. The gas and solid phase temperature solutions are linked via the boundary condition:
\be
   \dq'' = -k \, \frac{\partial \Tw(0,t)}{\partial x}   \label{bc}
\ee
where $\dq''$ is the net radiative and convective heat flux from the adjacent gas layer (see Chapter~\ref{chap:heat_transfer} for a description of the heat flux term).
The temperature within a solid of density, $\rho$, specific heat, $c$, and thermal conductivity, $k$, then evolves using this boundary condition according to the heat conduction equation:
\be
   \frac{\partial \Tw}{\partial t} = \frac{k}{c \, \rho} \frac{\partial^2 \Tw}{\partial x^2} \label{eq:fourier}
\ee

\section{Solution of the Equations}

CFAST uses the differential~/~algebraic solver DASSL~\cite{DASSL:1982,DASSL:1989} to solve the differential equations Eqs. (\ref{eq1}) through (\ref{eq4}) and the algebraic equation (once it is discretized) Eq. (\ref{bc}) for $P$, $\Tu$, $\Vu$,  $\Tl$ and $\Tw$ (at the surface only).
DASSL solves these equations by formulating them as a root finding problem
\begin{equation}
F\brackets{t,\vecy,\vecy'}=0
\end{equation}
where $t$ is the time,
$\vecy$ is the solution vector and $\vecy'$ is the solution vector derivative at time $t$.
...
The components of the solution vector derivative, $\vecy'$ are arranged analogously.

The components of the residual function $F$ are also arranged analogously to $\vecy$ and $\vecy'$. The first $n$ components correspond to the $dP/dt$ differential equations, the next $n$ to the $d\Tu/dt$ differential equations and so on.  Each component of $F$ is a residual which DASSL drives to zero.  For example if $dP_1/dt$, the differential equation for pressure in the first compartment, is computed using Eq. (\ref{eq1}), then  $F_1(t,\vecy,\vecy')$, the first component of $F$, is given by the difference between $y'_1$, the estimate of $dP_1/dt$ provided by DASSL, and the $dP_1/dt$ term computed from Eq. (\ref{eq1})  or equivalently

\begin{equation}
F_1(t,\vecy,\vecy')=\vecy'_1 - dP_1/dt
\label{eq:Fp1}
\end{equation}
DASSL then determines values for $\vecy$ and $\vecy'$ so that Eq. (\ref{eq:Fp1}) along with all other components of $F$ are sufficiently close to zero as determined by a user defined error criteria.
\renewcommand{\arraystretch}{1.5}
The definition of $F$ for all components is given by
\begin{equation}
...
\end{array}
\right.\label{eq:resid}
\end{equation}

The last equation, $q''(\vecy)+k\dbydx{\Tw}(0,t)$, is used to couple the gas with the solid.  It does this by requiring that the convective and radiative flux , $q''(\vecy)$, striking the wall surface  be consistent with the temperature gradient, $\dbydx{\Tw}(0,t)$, at that surface according to Fourier's law:
\begin{equation}
q''=k\dbydx{T}
\end{equation}

It is implemented as an algebraic equation by discretizing
the spatial derivative term $\dbydx{\Tw}$ and evaluated by advancing the wall temperature profile from a previously known solution at $t_{\rm old}$ to $t$ using Eq. (\ref{eq:fourier}) and the temperature boundary condition $\dbydx{\Tw}(0,t)=T_w$ provided by DASSL in $\vecy$.

One step in the solution process then is to advance the solution from $t_n$ to $t_{n+1}$ {\em i.e.}\ to find the solution of $F(t_{n+1},\vecy_{n+1},\vecy'_{n+1})=0$ given that the solution for
$F(t_{n},\vecy_{n},\vecy'_{n})=0$ is known.
The solver estimates values for $\vecy$ and $\vecy'$ at $t+\Delta t$. CFAST does this by calculating an actual $\vecy'$ from the estimated values of $\vecy$ and returns the difference between the estimated $\vecy'$ and calculated $\vecy'$ to DASSL using Eq. (\ref{eq:resid}). DASSL then advances the solution using an extension of Newton's method using derivatives that are approximated with backward differentiation formulae of orders one through five to solve for $\vecy$ and $\vecy'$ to within a specified tolerance~\cite{DASSL:1982,DASSL:1989}.
...
A flowchart describing the solution procedure follows

\begin{enumerate}
\item Assume that $\vecy_n$ and $\vecy'_n$ are known at $t=t_n$
\item For some $\Delta t$ guess values for $\vecy_{n+1}$ and $\vecy'_{n+1}$ at $t_{n+1}=t+\Delta t$
\label{step:init}
\item Using the guessed solution $\vecy_{n+1}$, compute mass flow rates: $\dml$ and $\dmu$ and heat flow rates: $\dql$ and $\dqu$ for each compartment
\label{step:flow}
\item Using the guessed solution $\vecy_{n+1}$ provided by DASSL and the mass and heat flow rate terms computed in step \ref{step:flow}, compute
...
\label{step:residode}
\item Compute the Fourier law residual (last row of Eq. (\ref{eq:resid}))
\label{step:residfourier}
\begin{enumerate}
\item Using the surface temperature found in $\vecy_{n+1}$ as a boundary condition, advance the temperature profile $\Tw$ from $t=t_n$ to $t=t_{n+1}$ using Eq. (\ref{eq:fourier})
\label{step:profile}
\item Compute the temperature gradient $\dbydx{\Tw}$ at the wall surface using the temperature profile computed in step \ref{step:profile}
\label{step:gradient}
\item Compute the residual $q''+k\dbydx{\Tw}$ using heat flow rates computed in step \ref{step:flow} and temperature gradient computed in step \ref{step:gradient}
\end{enumerate}
\item Are the residuals computed in steps \ref{step:residode} and \ref{step:residfourier} sufficiently small (according to a user specified error tolerance)?
