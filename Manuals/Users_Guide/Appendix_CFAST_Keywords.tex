...
\endhead
\parbox{1.5in}{\bf Parameter}    & \parbox{1in}{\bf Type}  & \parbox{1in}{\bf Reference}  & \parbox{1in}{\bf Units}  & \parbox{1in}{\bf Default Value} \\ \hline
{\ct F}*\footnote{ * indicates a required input for each {\ct RAMP} input included in the input file. Either {\ct T} or {\ct Z} (but not both) are required depending on the type associated with the ramp.}               & Real Array            & Section \ref{info:RAMP2}                 &          &                 \\ \hline
