...
The mass flows through the horizontal segments of the opening are distributed into the upper and lower layers of the downstream compartment. For the jet originating in the upper layer of the upstream compartment flowing into the upper layer of the downstream compartment, $\dm_{\rm u \to u}$, or the jet from lower layer to lower layer, $\dm_{\rm l \to l}$, the mass is applied directly to the downstream layer. However, consider the flow from the upper layer to the lower layer, $\dm_{\rm u \to l}$. Some of the mass is deposited directly in the lower layer, but some is assumed to rise into the upper layer in the form via a plume. The distribution of mass is calculated as follows. The enthalpy flow rate of the original jet is:
\be
   \doh_{\rm u \to l} = c_p \brackets{T_{\rm u,1}-T_{\rm l,2}} \, \dm_{\rm u \to l}
\ee
Assuming that $T_{\rm l,2} < T_{\rm u,1} < T_{\rm u,2}$, this energy will be distributed between the compartment's two layers (if not, the mass and energy go completely to the upper or layer layer). The distribution is based on the relative difference between the incoming jet temperature and the layer temperatures. Some of the energy stays within the lower layer and some of the energy rises into the upper layer in the form of a virtual plume. The effective heat release rate of this virtual plume is:
\be
