...
The compartment lining is divided into four surfaces: the ceiling, the floor, and the wall sections above and below the layer interface.  The outgoing radiant flux at surface $k$ consists of an emissive power and reflectance term and is given by
...
\noindent where $\qin$ is the radiant flux striking surface $k$ from all sources (wall segments, gas layers and fires).  The term $\qin$ is given by
