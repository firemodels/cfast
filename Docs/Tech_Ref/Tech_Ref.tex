...
The set of governing equations start with the conservation of mass which in words may be expressed as the net rate at which mass is added to a control volume, $\dot m_i$, is equal to the rate at which mass is accumulated in that control volume, $\dbydt{m_i}$ or equivalently
\be
   \dbydt{m_i} = \dot m_i  \label{mass_con}
\ee
where in particular $\dm_i$ represents the sum of all mass flow terms, such as plume mass entrainment and ventilation, entering and leaving layer $i$. Similarly, the conservation of energy may be stated as the net rate that heat is added to the control volume, $\qdot_i$ minus the work done by the control volume, $P\dbydt{V_i}$, (since the control volume may move), is equal to the rate at which heat or internal energy accumulates in the control volume, $\ddt\left(c_v m_i T_i\right)$  or equivalently
\be
...
For the derivations that follow, we assume that the pressure $P$ is the same for the upper and lower layer or equivalently $\rho_l T_l=\rho_u T_u$.

\subsection{Pressure Equation}
\noindent To derive a differential equation for compartment pressure, rewrite Eq. \ref{eq:first_law} for the upper and lower layer and substitute $\rho_i=m_i/V_i$ to give
\begin{eqnarray}
...
\noindent Sum Eqs. \ref{eq:lower} and \ref{eq:upper}, noting that $\dbydt{V_l}=-\dbydt{V_u}$ since $\ddt(V_l+V_u)=dV/dt=0$, and use the assumption that $\rho_l T_L=\rho_u T_U = P/R$ to give
\begin{eqnarray}
   \frac{c_vV}{R}\dbydt{ P}  &=  &\qdot_l + \qdot_u
\end{eqnarray}

\noindent Finally use $R=c_p-c_v$ and $\gamma=c_p/c_v$ to show that $R/c_v=\gamma-1$ giving
...
\subsection{Upper Layer Volume Equation}
...
\label{eq:vua}
\end{equation}
\noindent Substituting Eq. \ref{eq:vua} into the upper layer form Eq. \ref{eq:first_law} ($i=u$) gives
\begin{eqnarray}
\qdot_u&=&\frac{1}{\gamma-1}\ddt\left(V_uP\right) + P\dbydt{V_u}\\
&=&\frac{1}{\gamma-1}\left(P\dbydt{V_u} + V_u\dbydt{P}\right) + P\dbydt{V_u}\\
&=&\frac{\gamma}{\gamma-1}P\dbydt{V_u} + \frac{V_u}{\gamma-1}\dbydt{P}\label{eq:vub}
\end{eqnarray}

\noindent Finally, solving Eq. \ref{eq:vub} for $\dbydt{Vu}$ gives
\begin{equation}
...
$\ddt(c_vm_uT_u)=c_v\dot m_uT_u+c_vm_u\dbydt{T_u}$ and from Eq. \ref{eq:ODE_VU}
\begin{equation}
P\dbydt{Vu} = \frac{\gamma-1}{\gamma} \qdot - \frac{V_u}{\gamma}\dbydt{P}
\label{eq:pvu}
\end{equation}

\noindent Substituting these two expressions into the upper layer form of Eq. \ref{eq:first_law} (again $i=u$) gives

\begin{eqnarray}
   \ddt\left(c_v m_u T_u\right) +  P \dbydt{V_u} &=  &\qdot_u\\
c_v\dot m_uT_u+c_vm_u\dbydt{T_u}+\frac{\gamma-1}{\gamma} \qdot - \frac{V_u}{\gamma}\dbydt{P}&=&\qdot_u
\label{eq:tua}
\end{eqnarray}
...
\label{eq:ODE_TU}
\end{equation}
A differential equation for lower layer temperature
\begin{equation}
\dbydt{T_l}=\frac{1}{c_pm_l}\left( \qdot_u-c_p\dot m_lT_l+ V_l\dbydt{P}\right)
\label{eq:ODE_TL}
\end{equation}
...
A system of ordinary differential equations consisting of Eqs. \ref{eq:ODE_P}, \ref{eq:ODE_VU}, \ref{eq:ODE_TU} and \ref{eq:ODE_TL} for the compartment pressure, upper layer volume, and lower and upper layer temperature
...
The mass, $\dmau$ and $\dmal$, and heat flow, $\qdot_u$ and $\qdot_l$, terms are simply the net sum of all mass and heat flowing into a layer (through vents, from fires, between layers via a plume, or by radiation and convection from compartment surfaces).
