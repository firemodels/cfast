...
where $Q_f$ and $Q_{f,C}$ are the total and convective heat release rates, $D$ is the fire diameter, $z$ is the height above the base of the fire, and $T_\infty$, $c_p$, and $\rho_\infty$ are the temperature, heat capacity, and density of the ambient gas at height $z$. Depending on the location $z$, this may be either the lower gas layer temperature or upper gas layer temperature. Heskestad's correlation is used in CFAST whenever both the fire and point of interest are within a single layer. When a hot layer forms so that the fire source and target location are not in the same gas layer, the correlation must be modified since the plume now includes added enthalpy by entraining hot layer gas as it moves through the upper layer to the ceiling. Evans \cite{Evans:1984} defines a virtual source and heat release rate  to extend the plume into the upper layer. Evans' method defines the strength, $Q_{I,2}$ and location, $Z_{I,2}$ of the substitute source with respect to the interface between the upper and lower layers by
...
Heskestad's correlations are only valid above the mean flame height \cite{Heskestad:2002}. When the target position is below the mean flame height, CFAST uses the value calculated at the mean flame height as an estimate.  This is likely to underestimate the temperature within the flaming region.
