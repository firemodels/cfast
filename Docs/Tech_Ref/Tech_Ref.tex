...
\be
\qout{i}=\sigma\epsilon T_i^4+\left(1-\epsilon_i\right)\qin
\label{eq:qout}
\ee
where $\qin$ is the radiant flux striking surface $i$ from all sources (wall segments, gas layers and fires).  This incoming radiant flux is given by
\be
\qin=\sum_{j=1}^4\qout{j} \, F_{j-i} \, \tau_{j-i}+c''_i
\label{eq:qin}
\ee
...
The problem then is to determine the net radiative flux, $\dot{q}_{{\rm r},i}''$, at each of these surfaces.  This flux is defined as the difference between the incoming and outgoing radiative flux\footnote{The sign for the net radiative flux, $\dot{q}_{{\rm r},i}''$, is defined so that positive radiant fluxes heat up wall segments, consistent with how convective fluxes are defined.  This is opposite to the sign convention used in Siegel and Howell~\cite{SiegelandHowell:1981}}:
\be
\dot{q}_{{\rm r},i}'' = \qin - \qout{i}
\label{eq:raddiff}
\ee
Equation~(\ref{eq:raddiff}) is written as a system of four linear equations following Eq.~(17-20) in Siegel and Howell~\cite{SiegelandHowell:1981}:
\be
   -\frac{\dot{q}_{{\rm r},i}''}{\epsilon_i} + \displaystyle\sum_{j=1}^4 \frac{1 - \epsilon_j}{\epsilon_j} \;  F_{i-j} \; \tau_{i-j} \; \dot{q}_{{\rm r},j}'' = \sigma T_i^4 - \displaystyle\sum_{j=1}^4 \brackets{F_{i-j} \; \tau_{i-j} \; \sigma T_j^4 \, } - c''_i \label{RTE}
\ee
