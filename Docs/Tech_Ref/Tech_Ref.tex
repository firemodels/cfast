...
\newcommand{\qin}{q_{{\rm in},k}''}

The compartment lining is divided into four surfaces: the ceiling, the floor, and the wall sections above and below the layer interface.  The outgoing radiant flux at surface $k$ consists of an emissive power and reflectance term given by

\be
\qout{k}=\sigma\epsilon T_k^4+\left(1-\epsilon_k\right)\qin
\label{eq:qout}
\ee

\noindent where $\qin$ is the radiant flux striking surface $k$ from all sources (wall segments, gas layers and fires).  This incoming radiant flux is given by

\be
\qin=\sum_{j=1}^4\qout{j}F_{j-k}\tau_{j-k}+c''_k
\label{eq:qin}
\ee

\noindent The term $F_{j-k}$ is the configuration factor (fraction of radiant energy emitted by surface $j$ that is intercepted by surface $k$), $\tau_{j-k}$ is the transmittance, $\sigma$ is the Stefan-Boltzman constant, $\epsilon_k$ is the emissivity, $T_k$ is the temperature  and $c_k''$ is the radiative flux from gas layers and fire of surface $k$ .

The problem then is to determine the net radiative flux, $\dot{q}_{{\rm r},k}''$, at each of these surfaces.  This flux is defined as the difference between the incoming and outgoing radiative flux\footnote{The sign for the net radiative flux, $\dot{q}_{{\rm r},k}''$, is defined so that positive radiant fluxes heat up wall segments, consistent with how convective fluxes are defined .  This is opposite to the sign convention used in Siegel and Howell~\cite{SiegelandHowell:1981}}:

\be
\dot{q}_{{\rm r},k}'' = \qin - \qout{k}
\label{eq:raddiff}
\ee

\noindent Solving Eqs.~(\ref{eq:qout}), (\ref{eq:qin}) and (\ref{eq:raddiff})
for $\dot{q}_{{\rm r},k}''$ results in a simplified form of the radiation transport
equation given by Eq.~(17-20) in Siegel and Howell~\cite{SiegelandHowell:1981}:
\be
   -\frac{\dot{q}_{{\rm r},k}''}{\epsilon_k} + \displaystyle\sum_{j=1}^4 \frac{1 - \epsilon_j}{\epsilon_j} \;  F_{k-j} \; \tau_{k-j} \; \dot{q}_{{\rm r},j}'' = \sigma T_k^4 - \displaystyle\sum_{j=1}^4 \brackets{F_{k-j} \; \tau_{k-j} \; \sigma T_j^4 \, } - c''_k \label{RTE}
\ee

\noindent The radiation from the gases in the upper and lower layers and the fire is included in the last term:
\be
   c''_k = \displaystyle\sum_{j=1}^2 \epsilon_j \, F_{k-j} \, \sigma \, T_j^4 + \frac{\omega_{k-{\rm f}}}{A_k} \frac{\chi_{\rm r} \, \dQ}{4 \pi} \label{ckeq}
\ee
...
The net radiative flux, $\dq_{{\rm r},k}''$ found in Eq.~(\ref{RTE}), along with the convective flux discussed in Section~\ref{section:convection}\ is used as a boundary condition for computing heat conduction within the solid walls. The outgoing radiative flux, $\qout{k}$ is used in Section~\ref{section:target} for computing radiative flux to a target.  It may be rewritten in terms of known quantities, in particular the net radiative flux found in Eq. (\ref{RTE}), by eliminating $\qin$ in Eqs.~\ref{eq:qout} and~\ref{eq:raddiff} and is given by

\be
\qout{k}=\sigma T_k^4+\frac{1-\epsilon_k}{\epsilon_k}\dq_{{\rm r},k}''
\label{eq:qoutb}
\ee

\subsection{Solving the Radiation Equations}

Equation~(\ref{RTE}) is solved by noting that it is a system of linear equations of the form
\begin{equation}
Ax=Be-c
...
\end{eqnarray*}
The $k$'th component of the $n$-vector $e$ is $e_k=\sigma T_k^4$, $c_k$ is given by Eq.~(\ref{ckeq}) and the $k$'th component of the solution vector is $x_k=\dot{q}_{{\rm r},k}''$.  The term $\delta$ is the usual Kronekar delta function with
\begin{eqnarray*}
\delta_{k,j}=\left\{
\begin{array}{cc}
1&k=j\\
...
$\hat{q}_k= \dot{q}_{{\rm r},k}''/\epsilon_k$.  The coefficients $a_{k,j}$ then become
\begin{eqnarray*}
a_{k,j}&=&\delta_{k,j}-F_{k-j}\tau_{k-j}(1-\epsilon_j)
\end{eqnarray*}

The solution to the original problem is then $\dot{q}_{{\rm r},k}''=\hat{q}_k\epsilon_k$
where $\hat{q}_k$ is the solution to the transformed problem.
...
\be
   ...
   \displaystyle\sum_{f=1}^{n_{\rm fires}} \cos \, \theta_f \; \frac{\chi_{\rm r} \, \dQ_f}{4 \pi R_f^2}  + \displaystyle\sum_{j=1}^{10} \tau_j \, F_j \, \epsilon_j \, \qout{j}  +  \displaystyle\sum_{j=1}^2 \left(\tau_j \, F_j \, \epsilon_j \sigma \, T_j^4 \right)  -\sigma T_{\rm t}^4  \right)  \label{target_rad}
\ee
where $\epsilon_{\rm t}$ is the emissivity of the target, $\epsilon_{\rm j}$ is the emissivity of the wall surface, $\theta_f$ is the angle formed by the line of sight segment (between the fire $f$ and the target) of length $R_f$ and the normal direction to the target surface, $F_j$ and $F_{\rm u}$ are view factors for the walls and upper layer, $\tau_j$ and $\tau_{\rm u}$ are the values of transmittance for the path between the target and the walls and upper layer, $T_{\rm t}$ is the temperature of the target.  The term $\qout{j}$, given by Eq.~(\ref{eq:qoutb}), is the outgoing radiative flux at the $j$'th wall surface.
